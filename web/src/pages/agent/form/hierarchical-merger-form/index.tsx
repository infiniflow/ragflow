...
const HierarchicalMergerForm = ({ node }: INextOperatorForm) => {
  const { t } = useTranslation();
  const initialValues = useFormValues(initialGroupValues, node);
  ...
  useEffect(() => {
    ...
    if (method !== initialMode.current) {
      ...
      if (method === 'group') {
        ...
        form.reset({
          method: method,
          hierarchy: initialHierarchicalMergerValues.hierarchy,
          rules: initialHierarchicalMergerValues.rules,
        });
      }
    }
  }, [method, form]);
