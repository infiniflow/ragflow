...
**重要规则:**
  ...
  - **始终考虑**整个对话历史。`,
    ...
    setting: {
      ...
      keyInvalid: '您的 API 密钥无效。',
      citySenseBaseUrlPlaceholder: 'https://media-speech.ai.citysense.ru',
      citySenseBaseUrlTip: 'CitySense media-speech-mcp 基础 URL (Yandex SpeechKit 混合, 4h/1GB)',
      citySenseApiKeyInvalid: 'CitySense API 密钥无效 (X-Service-Token 不匹配)',
      citySenseConnectionFailed: '无法连接到 CitySense {url}',
      citySenseTranscriptionFailed: 'CitySense 转录失败',
      citySenseModelListFailed: '获取 CitySense 模型列表失败',
...
适用场景：具有流动性的、内容相关联的文档——如书籍、手册、报告和文章——其中相邻段落应保持在一起以维持叙述连贯性。`,
      ...
      autoPlay: '自动播放',
      emitAll: '显示所有消息块',
      emitAllTip: '开启后，此组件中的每个消息块都会依次作为单独的消息输出；关闭时，仅随机选择一个消息块。',
      thinking: '思考状态',
      thinkingTip: '开启后，此消息会在代理工作时显示为实时的“思考”指示器，并被下一条消息替换，而不是作为最终答案输出。',
      ...
      maxRetries: '最大重试轮数',
      toolTimeout: '工具调用超时',
      toolTimeoutTip: '单次工具调用（含 MCP 工具）的超时秒数。长时间运行的工具请调大该值。',
      maxSteps: '最大步数',
      headless: '无头模式',
