...
特定のナレッジベースを最初に検索したい場合は、他のものよりも高いページランクスコアを設定してください。`,
    ...
    fileManager: {
      ...
      uploadTitle: 'クリックまたはドラッグしてファイルをアップロード',
      uploadDescription:
        'RAGFlowは、単一またはバッチでのファイルアップロードをサポートします。ローカルデプロイ: 単一ファイルアップロードは1GB、バッチアップロードは32ファイルまで。アカウント全体のファイル数に制限はありません。demo.ragflow.io: バッチアップロードの合計サイズは10MB、ファイル数は128までです。',
      local: 'ローカルアップロード',
      s3: 'S3アップロード',
      preview: 'プレビュー',
      fileError: 'ファイルエラー',
