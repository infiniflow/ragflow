...
以上就是你需要總結的內容。`,
 ...
 NER：使用 spaCy NER 和基於規則的關鍵詞提取來抽取實體和關係，無需 LLM 參與提取過程，速度快且資源消耗低`,
    ...
    setting: {
      ...
      awsAuthModeBedrockAPIKey: 'Bedrock API 金鑰',
      bedrockAPIKey: 'Bedrock API 金鑰',
      bedrockAPIKeyMessage: '請輸入 Bedrock API 金鑰',
      bedrockEndpointType: '端點類型',
      bedrockEndpointRuntime: 'Bedrock Runtime',
      bedrockEndpointMantleOpenAI: 'Mantle（相容 OpenAI）',
      bedrockEndpointMantleAnthropic: 'Mantle（相容 Anthropic）',
      bedrockEndpointURL: '端點 URL',
      bedrockEndpointURLMessage: '請輸入可信任的 Bedrock HTTPS 端點',
      bedrockDiscoveryEndpointURL: '模型探索端點 URL',
      bedrockDiscoveryEndpointURLMessage:
        '可選，用於列出模型的 Bedrock 控制平面端點',
