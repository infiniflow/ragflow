...
Узел агрегации переменных (изначально узел назначения переменных) - это важный узел в рабочем процессе. Он отвечает за интеграцию выходных результатов разных ветвей, гарантируя, что независимо от того, какая ветвь выполнена, ее результат может быть ссылочно доступен через единую переменную. Это чрезвычайно полезно в сценариях с несколькими ветвями, так как он отображает переменные с той же функцией в разных ветвях на одну выходную переменную, избегая избыточных определений в нижестоящих узлах.`,
      ...
      tokenChunker: 'Токен-чанкер',
      tokenChunkerDescription:
        'Разбивает текст на чанки по количеству токенов с настраиваемыми разделителями и перекрытием.',
      titleChunker: 'Чанкер по заголовкам',
      titleChunkerDescription:
        'Разбивает документы на секции по иерархии заголовков. Уровни задаются регулярными выражениями, а режим определяет структуру чанков.',
      ...
      group: 'Группировка',
      hierarchy: 'Иерархия',
      addRegularExpressions: 'Добавить регулярное выражение',
      regularExpressions: 'Регулярные выражения',
      hierarchyTip: `Строит дерево заголовков и создаёт самодостаточные чанки с полным путём по иерархии, например: Часть 1 › Глава 3 › Раздел 2 + содержимое.

Подходит для строго структурированных текстов: законов, нормативных актов, договоров и технической документации, где важно положение каждого чанка в иерархии.`,
      groupTip: `Разбивает документ на плоские группы по выбранному уровню заголовков и объединяет соседние небольшие секции. Родительские заголовки в чанки не добавляются.

Подходит для книг, руководств, отчётов и статей, где важна смысловая связность соседних фрагментов.`,
      includeHeadingContent: 'Отделять содержимое родительских заголовков',
      includeHeadingContentTip:
        'Чанки будут содержать только путь заголовка и своё содержимое. Текст сразу после родительского заголовка будет сохранён в отдельном чанке.',
      rootAsHeading: 'Использовать первый чанк как глобальный контекст',
      rootAsHeadingTip:
        'Первый фрагмент будет использоваться как глобальный заголовок для всей иерархии. Это удобно для резюме и документов, где первая секция определяет предмет документа.',
      enableMultiColumn: 'Распознавать многоколоночную разметку',
      enableMultiColumnTip:
        'Определять колонки и сохранять правильный порядок чтения. Включите для документов с двумя колонками или газетной вёрсткой.',
      removeToc: 'Удалять исходное оглавление',
      removeTocTip:
        'Удалять оглавление исходного документа, чтобы оно не обрабатывалось как обычный текст и не попадало в чанки.',
      removeHeaderFooter: 'Удалять колонтитулы',
      flattenMediaToText: 'Отключить визуальную модель',
      flattenMediaToTextTip:
        'Обрабатывать изображения и таблицы как обычный текст без улучшения визуальной моделью.',
      extractAutomaticNumbering: 'Извлекать автоматическую нумерацию',
      extractAutomaticNumberingTip:
        'Приблизительно восстанавливать автоматическую нумерацию заголовков DOCX. Сложные правила перезапуска и подавления могут воспроизводиться неточно.',
      overlappedPercent: 'Процент перекрытия (%)',
