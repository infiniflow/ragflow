...
export default defineConfig(({ mode }) => {
  ...
  const proxySchemes = {
    ...
    hybrid: {
      ...
      '^(/api/v1/users)|^(/api/v1/auth)|^(/api/v1/users/me)|^(/api/v1/system/config)|^(/api/v1/system/version)|^(/api/v1/tenants)|^(/api/v1/chats)|^(/api/v1/searches)|^(/api/v1/files)|^(/api/v1/agents/[^/]+/upload$)':
        {
          target: 'http://127.0.0.1:9384/',
          changeOrigin: true,
          ws: true,
