...
func TestSplitLargeHTMLTableNoSplit(t *testing.T) {
	...
	parts, ranges := SplitLargeHTMLTable(small, len(small)+100, charTokens)
	if len(parts) != 1 || parts[0] != small || ranges != nil {
		t.Fatalf("small table must return unchanged with nil ranges, got %d parts ranges=%v", len(parts), ranges)
	}
	parts, ranges = SplitLargeHTMLTable("no table here", 1, charTokens)
	if len(parts) != 1 || parts[0] != "no table here" || ranges != nil {
...
func TestSplitLargeHTMLTableReplicatesHeaderAndSlicesRanges(t *testing.T) {
	...
	parts, ranges := SplitLargeHTMLTable(text, budget, charTokens)
	if len(parts) < 3 {
...
func TestSplitLargeHTMLTableHeaderlessReplicatesFirstRow(t *testing.T) {
	...
	b.WriteString("</table>")
	parts, ranges := SplitLargeHTMLTable(b.String(), len("<table><tr><td>r0</td></tr><tr><td>row1</td></tr></table>")+10, charTokens)
	if len(parts) < 2 {
