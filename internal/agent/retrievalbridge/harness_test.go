...
func TestBaseModelNameMirrorsPython(t *testing.T) {
	...
	for _, c := range cases {
		if got := common.BaseModelName(c.in); got != c.want {
...
// Exercise the production bridge: history must reach the graph and its
// formalizer rather than being replaced by only the latest question.
func TestHarnessRetrieverForwardsConversation(t *testing.T) {
	model := &historyModel{}
	called := false
	agentic_rag.SetAgenticLoop(func(ctx context.Context, deps agentic_rag.RAGTools, req agenticruntime.RunRequest, _ *agenticruntime.Kbinfos, resp *agentic_rag.RunResponse, _ *log.Logger) {
		called = true
		want := []schema.Message{{Role: schema.User, Content: "Tell me about Culdcept."}, {Role: schema.Assistant, Content: "Culdcept was made by OmiyaSoft."}, {Role: schema.User, Content: "When was it released?"}}
		if !reflect.DeepEqual(deps.Messages, want) {
			t.Fatalf("history lost: %#v", deps.Messages)
		}
		resp.Answer, _ = agentic_rag.Formalize(ctx, agenticruntime.SessionDeps{Model: model}, deps.Messages, 8192)
	})
	t.Cleanup(func() { agentic_rag.SetAgenticLoop(nil) })
	req := service.HarnessRequest{Question: "When was it released?", ThinkingMode: "high", Messages: []map[string]interface{}{
		{"role": "user", "content": "Tell me about Culdcept."},
		{"role": "assistant", "content": "Culdcept was made by OmiyaSoft."},
		{"role": "user", "content": []interface{}{map[string]interface{}{"type": "text", "text": "When was it released?"}}},
	}}
	got, err := NewHarnessRetriever(nil, nil, nil)(t.Context(), req)
	if err != nil {
		t.Fatal(err)
	}
	if !called || model.calls != 1 || got.Answer != "When was Culdcept released?" || !strings.Contains(model.transcript, "OmiyaSoft") {
		t.Fatalf("formalization lost conversation: result=%+v model=%+v", got, model)
	}
	req.Messages[0]["content"] = 17
	if _, err := NewHarnessRetriever(nil, nil, nil)(t.Context(), req); err == nil {
		t.Fatal("invalid history content accepted")
	}
}
...
func (m *historyModel) Complete(_ context.Context, messages []schema.Message, _ []agenticruntime.ToolSpec) (*agenticruntime.ModelReply, error) {
	m.calls++
	for _, msg := range messages {
		m.transcript += msg.Content
	}
	return &agenticruntime.ModelReply{Content: `{"question":"When was Culdcept released?","keywords":"Culdcept released"}`}, nil
}

func TestEmbeddingValidationPreservesAtInModelNames(t *testing.T) {
	if err := validateDatasetEmbeddingModels(t.Context(), []*entity.Knowledgebase{kb("model@q4@instance@provider", ""), kb("model@q8@instance@provider", "")}); err == nil {
		t.Fatal("different quantized models were conflated")
	}
}
