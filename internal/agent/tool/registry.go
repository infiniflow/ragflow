...
func buildRetrievalTool(params map[string]any) (einotool.BaseTool, error) {
	...
	for key := range params {
		switch key {
		case "dataset_ids", "kb_ids", "top_n", "prefetch_size", "top_k", "similarity_threshold",
	...
	if raw, exists := params["prefetch_size"]; exists {
		value, ok := strictInt(raw)
		if !ok || value <= 0 {
			return nil, fmt.Errorf("agent tool: retrieval tool requires positive integer node-level param prefetch_size")
		}
		defaults.PrefetchSize = value
	}
