...
// (the 22 plan components + the v1/tool wrappers including Wikipedia)
...
func TestVerifyRegistration_P1(t *testing.T) {
	...
	if got := len(names); got < 12 || got > 33 {
		t.Errorf("expected 12-33 registered (current plan scope + v1/tool wrappers), got %d: %v", got, names)
	}
