...
// TavilySearch, TavilyExtract, ExeSQL, Google, BGPT, YahooFinance, GoogleScholar, ArXiv,
...
// counts between 12 (P0+P1 minus the removed ExitLoop) and 37 (the 22 plan
...
func TestVerifyRegistration_P1(t *testing.T) {
	...
	if got := len(names); got < 12 || got > 37 {
		t.Errorf("expected 12-37 registered (current plan scope + v1 wrappers/stubs), got %d: %v", got, names)
	}
