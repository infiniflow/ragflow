...
// itself they could disagree — the union deduped ACROSS slots case-sensitively while the
// record line lowercased, so one entity written two ways by two sessions was one member to
// the record and two to the count.
...
//     count derived and the members accountable: parsing a count slot's text turns its
//     qualifiers into "members" while the real names sit in another slot.
...
//
// The last clause is what keeps a phrase out of the count. A session writing a place beside
// its owner, or an event beside its object, writes a token that is short, digit-free and
// unpunctuated — it passes every shape test a name passes — yet it is not a second member:
// the member it names is already in the table. It stays only until the last node is the
// sole writer of a member list: then a name the corpus does not state cannot enter one at
// all.
