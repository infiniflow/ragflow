...
type Kbinfos struct {
	...
	// (agentic_rag_graph.py:870). The renderer ranks the pool by similarity and
	// caps it (citeChunkCap), so this list is NOT Chunks in pool order, and a
	// "[ID:n]" the model writes refers to position n in THIS list — one entry per
	// RENDERED block, so a chunk the renderer skipped (no content) holds no
	// position here either. The chat pipeline resolves citations against it
