...
func SufficientContextAgent(ctx context.Context, deps SCADeps, question string, claims []ClaimDraft) SCAResult {
	...
	ctx, done := runtime.Phase(ctx, runtime.PhaseSCA)
	defer done()
...
func renderClaimContext(claims []ClaimDraft, kb *runtime.Kbinfos) string {
	if len(claims) == 0 {
...
// retrieval.chunkText (which prefers content and keeps a "text" fallback): the SCA evidence anchor
...
func coerceDict(v any) map[string]any {
	...
	case []any:
		...
		return nil
	case string:
		if parsed := runtime.ExtractJSON(t); parsed != nil {
