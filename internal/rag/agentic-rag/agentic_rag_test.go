...
func (m *streamingModel) Complete(_ context.Context, _ []schema.Message, _ []runtime.ToolSpec) (*runtime.ModelReply, error) {
	if m.fail {
	...
	return &runtime.ModelReply{Content: out}, nil
}

func (m *streamingModel) StreamComplete(_ context.Context, _ []schema.Message, _ []runtime.ToolSpec, onDelta func(string, bool) error) (*runtime.ModelReply, error) {
	if m.fail {
...
func TestOuterReactSessionPublishMergesPerCallResults(t *testing.T) {
	session := &outerReactSession{kb: &runtime.Kbinfos{}, resp: &RunResponse{}}
	const calls = 4
...
func TestOuterReactSessionNarratesToolCalls(t *testing.T) {
	...
	session := &outerReactSession{
		...
		req:    runtime.RunRequest{Question: "q"},
	}
...
func TestOuterReactSessionUnknownToolNarrates(t *testing.T) {
	var buf strings.Builder
	events := make(chan runtime.ThinkEvent, 4)
	spec := runtime.GetMode("naive")
	session := &outerReactSession{
		ctx: context.Background(), spec: spec, kb: &runtime.Kbinfos{},
		resp: &RunResponse{Mode: spec}, logger: log.New(&buf, "", 0),
		req: runtime.RunRequest{Question: "q"},
	}
	ctx := runtime.WithSteps(context.Background(), runtime.StepReporter{
		Events: func(ev runtime.ThinkEvent) { events <- ev },
	})
	session.ctx = ctx
	...
	var result runtime.ThinkEvent
	for len(events) > 0 {
		ev := <-events
		if ev.Kind == runtime.ThinkKindToolResult {
			result = ev
		}
	}
	if result.Status != runtime.StatusMiss || result.Reason != runtime.ReasonUnwired {
...
// developer log, not the think block. It describes the runtime' wiring rather than
...
func TestOuterLoopAbsentLineStaysOutOfTheThinkBlock(t *testing.T) {
	...
	Rag(ctx, RAGTools{Logger: log.New(&logged, "", 0)}, runtime.RunRequest{
		Question:     "anything",
		ThinkingMode: "low",
...
func TestRagToolResultLine(t *testing.T) {
	...
	cases := []struct {
		...
		{"grounded", &RunResponse{Answer: "cited answer"}, &runtime.Kbinfos{Chunks: twoChunks}, label,
			`The rag tool returned a cited answer grounded in 2 passages for "曹操是谁".`},
		{"grounded-singular", &RunResponse{Answer: "a"}, &runtime.Kbinfos{Chunks: twoChunks[:1]}, label,
			`The rag tool returned a cited answer grounded in 1 passage for "曹操是谁".`},
		{"no-evidence", &RunResponse{Answer: "answer"}, &runtime.Kbinfos{}, label,
			`The rag tool returned no answer for "曹操是谁": research gathered no evidence.`},
		{"evidence-but-no-answer", &RunResponse{}, &runtime.Kbinfos{Chunks: twoChunks}, label,
			`The rag tool gathered 2 passages but composed no answer for "曹操是谁".`},
		{"unlabelled", &RunResponse{Answer: "a"}, &runtime.Kbinfos{Chunks: twoChunks[:1]}, "",
...
func TestOuterReactSessionToolCallWaitsOnInFlightRag(t *testing.T) {
	...
	session := &outerReactSession{
		...
		resp:   &RunResponse{Mode: spec},
		logger: log.New(io.Discard, "", 0),
		req:    runtime.RunRequest{Question: "original question"},
	}
...
func TestOuterReactSessionSelectsWinningCallEvidence(t *testing.T) {
	...
	loser := &runtime.Kbinfos{Chunks: []map[string]any{{"chunk_id": "l0"}}}
...
func TestOuterReactSessionSelectEvidenceKeepsUnionWithoutAMatch(t *testing.T) {
	session := &outerReactSession{kb: &runtime.Kbinfos{}, resp: &RunResponse{}}
	session.publish(&RunResponse{Answer: "answer-a"}, &runtime.Kbinfos{Chunks: []map[string]any{{"chunk_id": "a0"}}})
