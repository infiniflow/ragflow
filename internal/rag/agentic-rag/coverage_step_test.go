...
func TestCoverageResolveCandidatesJudgeWhatTheRunTouched(t *testing.T) {
	...
	kb.StoreCoverageSet(runtime.CoverageSet{
		Operands: []string{"云长", "斩"},
		Windows:  []runtime.CoverageWindow{{ChunkID: "w1", Quote: "云长手起刀落，斩孔秀于马下"}},
	})
	value := slots.Items(slots.Item{Value: "文丑", ChunkID: "c9", Quote: "云长诛文丑"})
	table := runtime.NewState([]runtime.Variable{{ID: 0, Type: "person", Value: &value}}, 0, nil)

	set, added, claims := coverageResolveCandidates(kb, &table, runtime.Coverage{})
...
func TestCoverageResolveCandidatesSkipWhatIsAlreadyQuoted(t *testing.T) {
	...
	kb.StoreCoverageSet(runtime.CoverageSet{
		Windows: []runtime.CoverageWindow{{ChunkID: "c1", Quote: "云长提华雄之头，掷于地上"}},
	})

	set, added, claims := coverageResolveCandidates(kb, nil, runtime.Coverage{})
...
func TestCoverageResolveCandidatesReopenAClaimNobodyJudged(t *testing.T) {
	...
	table := runtime.NewState([]runtime.Variable{{ID: 1, Type: "person", Value: &claimed}}, 0, nil)
	...
	settled := runtime.NewState([]runtime.Variable{{ID: 1, Type: "person", Value: &anchored}}, 0, nil)
	if _, again, _ := coverageResolveCandidates(kb, &settled, runtime.Coverage{}); again != 0 {
...
func TestCoverageResolveCandidatesHandAClaimToTheVerdict(t *testing.T) {
	...
	set, reached, claims := coverageResolveCandidates(kb, &table, runtime.Coverage{Acts: []string{"斩"}, Actor: "关羽"})
...
func TestRunCoverageResolveWritesWhatTheRunTouched(t *testing.T) {
	...
	names := runtime.ItemValues(&st.SlotTable)
	found := false
