...
func TestBatchFillSlots_ClustersOverlappingEvidence(t *testing.T) {
	...
	deps := runtime.SessionDeps{Model: mdl, KB: &runtime.Kbinfos{Chunks: chunks}}
...
func TestBatchFillSlots_ThresholdsBlockMerging(t *testing.T) {
	mk := func(e1, e2 []string) (runtime.State, map[string]SlotEvidence) {
		st := runtime.NewState([]runtime.Variable{
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			mdl := &scriptedModel{}
			deps := runtime.SessionDeps{Model: mdl, KB: &runtime.Kbinfos{}}
...
func TestBatchFillSlots_MaxSlotsPerBatch(t *testing.T) {
	vars := make([]runtime.Variable, 0, 5)
	ev := map[string]SlotEvidence{}
	for i := 1; i <= 5; i++ {
		vars = append(vars, runtime.Variable{ID: i, Type: "aspect", QuestionClues: []string{"same clue"}})
		ev[fmt.Sprint(i)] = SlotEvidence{EvidenceIDs: []string{"c1", "c2"}}
	}
	st := runtime.NewState(vars, 0, nil)
	kb := &runtime.Kbinfos{Chunks: []map[string]any{
...
func TestBatchFillSlots_LLMFailureBestEffort(t *testing.T) {
	mk := func() (runtime.State, map[string]SlotEvidence) {
		st := runtime.NewState([]runtime.Variable{
			{ID: 1, Type: "aspect", QuestionClues: []string{"a"}},
			{ID: 2, Type: "aspect", QuestionClues: []string{"b"}},
		}, 0, nil)
	...
	t.Run("no-model", func(t *testing.T) {
		st, ev := mk()
		deps := runtime.SessionDeps{KB: kb} // Model nil — nothing can be filled
		if got := BatchFillSlots(context.Background(), deps, &st, ev); got != 0 {
			t.Fatalf("filled = %d, want 0", got)
		}
	})
