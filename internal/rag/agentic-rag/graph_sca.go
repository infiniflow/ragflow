...
func SelectSCAView(chunks []map[string]any, focusTerms []string) ([]map[string]any, string) {
	...
	for _, r := range ranked {
		if strings.HasPrefix(runtime.ChunkIDOf(r.chunk), evidenceChunkPrefix) {
...
func scaNode(ctx context.Context, deps RAGTools, st *AgenticState, logger *log.Logger) {
	ctx, done := runtime.Phase(ctx, "sca")
	defer done()
	...
	step(ctx, logger, "SCA", "Evidence check: %s (confidence %.2f, reviewed %d of %s).",
		st.Verdict, res.Confidence, len(view), runtime.CountOf(len(chunks), "passage"))
}
	...
	for _, sid := range sids {
		...
		for _, eid := range meta.EvidenceIDs {
			...
			if p, ok := viewIndexByID[eid]; ok {
				...
				if p, ok := viewIndexByID[runtime.ChunkIDOf(c)]; ok {
					pos = p
				} else {
					view = append(view, c)
					pos = len(view) - 1
...
func resolveEvidenceChunk(eid string, chunks []map[string]any) map[string]any {
	...
	for _, c := range chunks {
		if runtime.ChunkIDOf(c) == eid {
			return c
		}
		if id := runtime.DocIDOf(c); id != "" && id == eid {
			return c
		}
	}
