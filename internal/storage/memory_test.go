...
func TestMemoryStorage_PutGet(t *testing.T) {
	...
	if err := ms.Put(ctx, "b1", "k1", payload); err != nil {
		t.Fatalf("Put returned error: %v", err)
	}

	got, err := ms.Get(ctx, "b1", "k1")
	if err != nil {
...
func TestMemoryStorage_GetMissing(t *testing.T) {
	...
	if _, err := ms.Get(ctx, "missing-bucket", "k"); !errors.Is(err, ErrMemoryNotFound) {
		t.Fatalf("Get on missing bucket: expected ErrMemoryNotFound, got %v", err)
	}

	if err := ms.Put(ctx, "b1", "exists", []byte("data")); err != nil {
		t.Fatalf("Put failed: %v", err)
	}
	if _, err := ms.Get(ctx, "b1", "missing-key"); !errors.Is(err, ErrMemoryNotFound) {
...
func TestMemoryStorage_Remove(t *testing.T) {
	...
	if err := ms.Remove(ctx, "ghost", "k"); err != nil {
	...
	if err := ms.Remove(ctx, "b1", "k1"); err != nil {
...
func TestMemoryStorage_RemoveBucket(t *testing.T) {
	...
	for _, k := range []string{"a", "b", "c"} {
		if err := ms.Put(ctx, "b1", k, []byte(k)); err != nil {
			t.Fatalf("Put failed: %v", err)
		}
	}
	if err := ms.Put(ctx, "b2", "x", []byte("x")); err != nil {
		t.Fatalf("Put failed: %v", err)
	}

	if err := ms.RemoveBucket(ctx, "b1"); err != nil {
		t.Fatalf("RemoveBucket failed: %v", err)
	}
	if ms.BucketExists(ctx, "b1") {
		t.Fatalf("BucketExists returned true after RemoveBucket")
	}
	if !ms.BucketExists(ctx, "b2") {
...
func TestMemoryStorage_CopyMove(t *testing.T) {
	...
	if err := ms.Put(ctx, "src", "k", []byte("payload")); err != nil {
		t.Fatalf("Put failed: %v", err)
	}

	// Copy preserves source.
	if !ms.Copy(ctx, "src", "k", "dst", "k2") {
		t.Fatalf("Copy returned false on existing source")
	}
	if !ms.ObjExist(ctx, "src", "k") {
		t.Fatalf("source missing after Copy")
	}
	if !ms.ObjExist(ctx, "dst", "k2") {
		t.Fatalf("destination missing after Copy")
	}
	got, err := ms.Get(ctx, "dst", "k2")
	if err != nil {
...
func TestMemoryStorage_BucketExists(t *testing.T) {
	...
	if err := ms.Put(ctx, "b1", "k", []byte("v")); err != nil {
		t.Fatalf("Put failed: %v", err)
	}
	if !ms.BucketExists(ctx, "b1") {
...
func TestMemoryStorage_PresignedURL(t *testing.T) {
	...
	url, err := ms.GetPresignedURL(ctx, "b1", "k1", time.Minute)
	if err != nil {
	...
	if _, err := ms.GetPresignedURL(ctx, "b1", "missing", time.Minute); !errors.Is(err, ErrMemoryNotFound) {
...
func TestMemoryStorage_Concurrent(t *testing.T) {
	...
	for i := 0; i < writers; i++ {
		go func(i int) {
			...
			if err := ms.Put(ctx, "race", key, payload); err != nil {
...
func TestMemoryStorage_Inspect(t *testing.T) {
	...
	if err := ms.Put(ctx, "b1", "k1", []byte("12345")); err != nil {
		t.Fatalf("Put failed: %v", err)
	}
	if err := ms.Put(ctx, "b1", "k2", []byte("hello")); err != nil {
		t.Fatalf("Put failed: %v", err)
	}
	if err := ms.Put(ctx, "b2", "only", []byte("x")); err != nil {
		t.Fatalf("Put failed: %v", err)
	}
	...
	if err := ms.RemoveBucket(ctx, "b2"); err != nil {
		t.Fatalf("RemoveBucket failed: %v", err)
	}
	if got := ms.Inspect(); len(got) != 0 {
