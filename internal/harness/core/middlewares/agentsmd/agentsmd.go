...
func (m *middleware[M]) BeforeAgent(ctx context.Context, rc *core.ReActAgentContext) (context.Context, *core.ReActAgentContext, error) {
	...
	for _, file := range m.cfg.Files {
		loaded, err := m.loadFile(file, make(map[string]bool), &totalBytes, 0)
		if err != nil {
	...
	if sess != nil {
		...
		_ = core.SetRunLocalValue(ctx, "_agentsmd_injected", true)
	}
...
func getSession(ctx context.Context) *runCtx {
	...
	v, ok, _ := core.GetRunLocalValue(ctx, "_agentsmd_injected")
	if ok {
...
func (m *middleware[M]) loadFile(path string, visited map[string]bool, totalBytes *int, depth int) (string, error) {
	// Cache check
	m.mu.Lock()
	...
	result, err := m.processContent(content, depth, visited, totalBytes)
	...
	for _, line := range lines {
		...
		if matches := importRegex.FindStringSubmatch(trimmed); len(matches) > 1 && m.cfg.Backend != nil {
			...
			if m.cfg.Backend.Exists(importPath) {
				imported, err := m.loadFile(importPath, visited, totalBytes, depth+1)
				if err != nil {
					if m.cfg.OnLoadWarning != nil {
