			...
			if useReasoning && chatModel != nil && len(kbs) > 0 {
				...
				if hErr != nil {
					...
					if harnessAnswer != "" {
						...
						final := s.decorateHarnessAnswer(harnessAnswer, kbinfos, slotCites, quote)
						final.Final = true
						out <- final
						return
					}
...
// decorateHarnessAnswer formats the reasoning chat's final answer. When quoting
// is enabled it resolves the harness citations and builds their references;
// otherwise it removes citations and explicitly clears the reference payload.
func (s *ChatPipelineService) decorateHarnessAnswer(answer string, kbinfos map[string]interface{}, slotCitations map[string][]string, quote bool) AsyncChatResult {
	think := ""
	ans := answer
	if strings.Contains(answer, "</think>") {
