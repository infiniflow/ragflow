...
// captureFilterFetcher wraps mockVectorFetcher and captures the filter
// from the SearchRequest for inspection.
type captureFilterFetcher struct {
	mockVectorFetcher
	capturedFilter *map[string]interface{}
}

func (c *captureFilterFetcher) Search(ctx context.Context, req *types.SearchRequest) (*types.SearchResult, error) {
	if c.capturedFilter != nil {
		*c.capturedFilter = req.Filter
	}
	return c.mockVectorFetcher.Search(ctx, req)
}
...
func TestFetchChunkVectors_EmptyInput(t *testing.T) {
	...
	// Empty slice also returns empty map.
	mock = &mockVectorFetcher{engineType: "elasticsearch"}
...
func TestFetchChunkVectors_JSONNumberVector(t *testing.T) {
	// Simulates Infinity returning json.Number in []interface{}.
	mock := &mockVectorFetcher{
		engineType: "elasticsearch",
		searchResults: map[string]*types.SearchResult{
			"ragflow_t1": {
...
func TestFetchChunkVectors_WrongDimVectorReturnsZero(t *testing.T) {
	...
	zero := make([]float64, 3)
	if !reflect.DeepEqual(result["c1"], zero) {
		t.Errorf("expected zero for wrong-dim input, got %v", result["c1"])
	}

	// Also test []float64 with wrong dim.
	mock = &mockVectorFetcher{
		engineType: "elasticsearch",
		searchResults: map[string]*types.SearchResult{
			"ragflow_t1": {
...
func TestFetchChunkVectors_FilterIsSliceOfInterface(t *testing.T) {
	// The ES filter builder only handles []interface{} for the "id" key.
	// Passing []string would be silently dropped.
	var capturedFilter map[string]interface{}
	mock := &captureFilterFetcher{
		mockVectorFetcher: mockVectorFetcher{
			engineType: "elasticsearch",
			searchResults: map[string]*types.SearchResult{
				"ragflow_t1": {
					Chunks: []map[string]interface{}{
...
func TestFetchChunkVectors_EmptyTenantIDs(t *testing.T) {
	// Empty tenantIDs → for loop does not execute, all chunks get zero vectors.
	mock := &mockVectorFetcher{engineType: "elasticsearch"}
	...
	zero := make([]float64, 3)
	if !reflect.DeepEqual(result["c1"], zero) {
		t.Errorf("c1 should be zero, got %v", result["c1"])
	}
	if !reflect.DeepEqual(result["c2"], zero) {
		t.Errorf("c2 should be zero, got %v", result["c2"])
	}
...
func TestFetchChunkVectors_NilKbIDs(t *testing.T) {
	// Nil kbIDs should not cause a panic; search should still work.
	mock := &mockVectorFetcher{
		engineType: "elasticsearch",
		searchResults: map[string]*types.SearchResult{
			"ragflow_t1": {
...
func TestParseVectorField_Float64SliceIsIndependent(t *testing.T) {
	// Verify that the returned []float64 is a copy, not an alias into the chunk map.
