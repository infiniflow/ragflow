...
func (s *ChatSessionService) asyncChatSolo(dialog *entity.Chat, session *entity.ChatSession, messages []map[string]interface{}, config map[string]interface{}, messageID string, reference []interface{}, stream bool) (map[string]interface{}, error) {
	...
	processedMessages = s.applyPromptTransforms(context.Background(), dialog, processedMessages, chatModel)
...
func (s *ChatSessionService) applyPromptTransforms(ctx context.Context, dialog *entity.Chat, messages []map[string]interface{}, chatModel *modelModule.ChatModel) []map[string]interface{} {
	if dialog.PromptConfig == nil || chatModel == nil {
		return messages
	}
	...
	transformed := copyChatSessionMessages(messages)
	question := messageContentText(transformed[latestIdx]["content"])
	if question == "" {
		return transformed
	}

	if promptConfigBool(dialog.PromptConfig, "refine_multiturn") && userMessageCount(transformed) > 1 {
		refined, err := FullQuestion(ctx, chatModel, transformed, promptConfigString(dialog.PromptConfig, "language"))
		if err != nil {
	...
	if languages := promptConfigStringSlice(dialog.PromptConfig, "cross_languages"); len(languages) > 0 {
	...
	if promptConfigBool(dialog.PromptConfig, "keyword") {
...
func copyChatSessionMessages(messages []map[string]interface{}) []map[string]interface{} {
	copied := make([]map[string]interface{}, len(messages))
	for i, msg := range messages {
		copied[i] = make(map[string]interface{}, len(msg))
		for key, value := range msg {
