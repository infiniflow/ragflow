...
func TestPipeline_CompareWithPython(t *testing.T) {
	...
	var goBoxes []TextBox
	for pg := 0; pg <= 1; pg++ {
		bxs := p.charsToBoxes(pageGroups[pg], pg)
	...
	goSections := p.boxesToSections(goBoxes)
...
func (m *mockEngine) ExtractTables(pageNum int) ([]TableRegion, error) { return nil, nil }
...
func TestParser_Parse(t *testing.T) {
	...
	cfg := DefaultConfig()
	p := NewParser(cfg)
	sections, _, err := p.Parse(engine)
	if err != nil {
		t.Fatalf("Parse: %v", err)
	}
	pySections := loadPySections(t, "py_sections.json")
	t.Logf("Parse: Go=%d sections, Python=%d sections", len(sections), len(pySections))
	if len(sections) == 0 {
	...
	if len(sections) != len(pySections) {
		t.Logf("section count: Go=%d Python=%d (different merge behavior expected)", len(sections), len(pySections))
	}
...
func TestEndToEnd_SameChars(t *testing.T) {
	...
	for _, name := range []string{
		...
		t.Run(name, func(t *testing.T) {
			...
			if len(meanH) == 0 { meanH = calcMedianCharHeights(allChars) }
			if len(meanW) == 0 { meanW = calcMedianCharWidths(allChars) }

			p := NewParser(DefaultConfig())
			pageGroups := groupCharsByPage(allChars)
			var goBoxes []TextBox
			for pg, chars := range pageGroups {
				goBoxes = append(goBoxes, p.charsToBoxes(chars, pg)...)
			}
			...
			sections := p.boxesToSections(goBoxes)
...
func snapshotCharsToGo(pageChars [][]json.RawMessage) []TextChar {
	...
	for _, page := range pageChars {
		for _, raw := range page {
			...
			if err := json.Unmarshal(raw, &c); err != nil { continue }
...
func groupCharsByPage(chars []TextChar) map[int][]TextChar {
	...
	for _, c := range chars { g[c.PageNumber] = append(g[c.PageNumber], c) }
	return g
}

func pageMeanFloat(nums []float64) map[int]float64 {
	r := make(map[int]float64)
	for i, v := range nums { r[i] = v }
	return r
}
...
func calcMedianCharHeights(chars []TextChar) map[int]float64 {
	...
	r := make(map[int]float64)
	for pg, ch := range g { r[pg] = MedianCharHeight(ch) }
	return r
}
...
func calcMedianCharWidths(chars []TextChar) map[int]float64 {
	g := groupCharsByPage(chars)
	r := make(map[int]float64)
	for pg, ch := range g { r[pg] = MedianCharWidth(ch) }
	return r
}
