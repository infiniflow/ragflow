...
// Corresponds to Python's manual_value_resolver: Callable[[dict], dict]
...
// Corresponds to rag/prompts/generator.py which uses PROMPT_JINJA_ENV.from_string(META_FILTER).render()
...
// Corresponds to rag/prompts/meta_filter.md and rag/prompts/generator.py:gen_meta_filter()
...
// Corresponds to rag/prompts/generator.py:gen_meta_filter().
//
// Uses ChatToModelByApiKey via ModelCredentials to call the LLM with a rendered prompt
// from meta_filter.md template. Returns filter conditions (key, value, op) for metadata filtering.
func GenMetaFilter(ctx context.Context, creds *entity.ModelCredentials, metaData map[string]interface{}, question string, constraints map[string]string) (*MetaFilterResult, error) {
	...
	response, code, err := modelProviderSvc.ChatToModelByApiKey(creds.ProviderName, creds.ModelName, creds.APIKey, fullPrompt)
	if err != nil {
		logger.Warn("ChatToModelByApiKey failed for GenMetaFilter",
...
// Corresponds to common/metadata_utils.py:meta_filter()
...
func ApplyMetaDataFilter(
	...
	case "manual":
		...
		// Apply manual_value_resolver callback if provided (Python: if manual_value_resolver: filters = [manual_value_resolver(flt) for flt in filters])
