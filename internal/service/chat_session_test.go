...
func TestApplyPromptTransformsRefinesTranslatesAndAddsKeywords(t *testing.T) {
	...
	transformed := svc.applyPromptTransforms(context.Background(), dialog, messages, newFakeChatModel(driver))
...
func TestApplyPromptTransformsFallsBackOnIncompleteModel(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			svc := &ChatSessionService{}
			transformed := svc.applyPromptTransforms(context.Background(), dialog, messages, tt.chatModel)
