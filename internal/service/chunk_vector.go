...
func FetchChunkVectors(ctx context.Context, engine vectorFetcher, chunkIDs, tenantIDs, kbIDs []string, dim int) map[string][]float64 {
	...
	for _, tid := range tenantIDs {
		...
		if err != nil {
			common.Warn("FetchChunkVectors search failed",
		...
		for _, chunk := range res.Chunks {
			...
			v := parseVectorField(chunk, vecField, dim)
			if v == nil {
				v = zeroVector(dim)
			}
			out[cid] = v
...
// Each call returns an independent backing array so callers can safely mutate
// individual vectors without aliasing.
func zeroVector(dim int) []float64 {
	return make([]float64, dim)
}

// parseVectorField extracts a vector of the expected dimension from a chunk
// map.  Returns nil when the field is missing, unparseable, or has the
// wrong dimension.
