...
func MergeTablesAcrossPages(tables []pdf.TableItem, medianHeights, pageHeights map[int]float64) []pdf.TableItem {
	...
	return deduplicateTableFragments(result)
}

type tablePositionKey struct {
	page                     int
...
func deduplicateTableFragments(tables []pdf.TableItem) []pdf.TableItem {
	...
	positions := make([]map[tablePositionKey]int, len(tables))
	positionCounts := make([]int, len(tables))
	rows := make([][]string, len(tables))
	for i, item := range tables {
		positions[i], positionCounts[i] = tablePositionCounts(item.Positions)
		rows[i] = tableRows(item.Grid)
	}
	dropped := make([]bool, len(tables))
	for i := range tables {
		if dropped[i] {
			continue
		}
		for j := i + 1; j < len(tables); j++ {
			...
			if strictlyContainsTableContent(positions[i], positionCounts[i], rows[i], positions[j], positionCounts[j], rows[j]) {
				dropped[j] = true
			} else if strictlyContainsTableContent(positions[j], positionCounts[j], rows[j], positions[i], positionCounts[i], rows[i]) {
				dropped[i] = true
				break
			}
		}
	}
	out := make([]pdf.TableItem, 0, len(tables))
	for i, item := range tables {
		if !dropped[i] {
			out = append(out, item)
		}
...
func strictlyContainsTableContent(parentPositions map[tablePositionKey]int, parentPositionCount int, parentRows []string,
	childPositions map[tablePositionKey]int, childPositionCount int, childRows []string) bool {
	if childPositionCount == 0 || childPositionCount >= parentPositionCount || len(childRows) == 0 || len(childRows) > len(parentRows) {
		return false
	}
	for key, count := range childPositions {
		if parentPositions[key] < count {
			return false
		}
	}
	return tableRowsContained(parentRows, childRows)
}

func tablePositionCounts(positions []pdf.Position) (map[tablePositionKey]int, int) {
	counts := make(map[tablePositionKey]int)
	total := 0
	for _, p := range positions {
		...
		for _, page := range pages {
			key := tablePositionKey{page: page, left: p.Left, right: p.Right, top: p.Top, bottom: p.Bottom}
			counts[key]++
			total++
		}
	}
...
func tableRows(grid [][]pdf.TSRCell) []string {
	...
	for _, row := range grid {
		...
		if signature := text.String(); signature != "" {
			rows = append(rows, signature)
		}
	}
	return rows
}

func tableRowsContained(parentRows, childRows []string) bool {
	matched := make([]bool, len(parentRows))
	for _, child := range childRows {
		match := -1
		for i, parent := range parentRows {
			if !matched[i] && parent == child {
				match = i
				break
			}
		}
		if match < 0 {
			for i, parent := range parentRows {
				if matched[i] || !strings.Contains(parent, child) {
					continue
				}
