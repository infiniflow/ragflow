...
func TestChatChannelDAO_CRUD(t *testing.T) {
	...
	err := dao.Create(ctx, cc)
	if err != nil {
		t.Fatalf("failed to create chat channel: %v", err)
	}

	// 2. Test GetByID
	res, err := dao.GetByID(ctx, "chan-1", "tenant-1")
	if err != nil {
	...
	_, err = dao.GetByID(ctx, "chan-1", "tenant-2")
	if err == nil {
	...
	err = dao.UpdateByID(ctx, "chan-1", "tenant-2", updates)
	if err != nil {
	...
	err = dao.UpdateByID(ctx, "chan-1", "tenant-1", updates)
	if err != nil {
...
func TestChatChannelDAO_ListByTenantID(t *testing.T) {
	...
	// Perform query
	ctx := t.Context()
	list, err := dao.ListByTenantID(ctx, "tenant-1")
	if err != nil {
