...
// The current reTableRowHdr regex only matches ".* (row|header)" — it misses
...
func TestGroupTSRCellsToRowsLabeled_DefaultTableLabel(t *testing.T) {
	...
	if len(rows) != 2 {
		t.Fatalf("label %q: expected 2 rows, got %d (BUG: reTableRowHdr does not match %q)", "table", len(rows), "table")
	}
	if len(rows[0]) != 2 || len(rows[1]) != 2 {
