...
// =============================================================================
// Issue 5: proj_match patterns — TDD fix
// Python proj_match (pdf_parser.py:1248) has 13 priority-ranked patterns plus
// two pre-filter rules. Go MatchProj must match all of them.
// =============================================================================

// TestMatchProj_AllPythonPatterns covers all 13 patterns from Python's
// proj_match (pdf_parser.py:1248-1270), including the two pre-filter rules.
// Pattern reference: each test case annotated with Python priority (1-12).
func TestMatchProj_AllPythonPatterns(t *testing.T) {
	tests := []struct {
		text string
		want bool // expected Go MatchProj result (matches Python truthiness)
	}{
		// ── Pre-filter: len ≤ 2 → no match ──
		{"AB", false},
		{"一", false},
		{"1", false},
		{"", false},

		// ── Pre-filter: pure digits/spaces/punctuation → no match ──
		{"123", false},
		{"1.2", false},
		{"+-*/", false},
		{"1, 2, 3", false},

		// ── Priority 1: 第[零一二...百]+章 ──
		{"第一章 概述", true},
		{"第十二章 财务分析", true},

		// ── Priority 2: 第[零一二...百]+[条节] ──
		{"第五条 合同条款", true},
		{"第十条", true},
		...
		{"1、说明", true},
		{"1. 引言", true},
		{"1.概述", true},     // dot followed by non-digit/non-dot char
		...
		{"What is this?", true},
		{"结论与展望：", true},
		// Too long (>48 chars ending with colon/question) → no match
		{"这是一个非常非常非常非常非常非常非常非常非常非常非常非常非常非常非常非常非常非常非常非常非常非常长的句子结尾呢？", false},
...
// TestMatchProj_ReturnTypeDifference documents that Go returns bool while
// Python returns int priority. Python's __filterout_scraps only uses the
// truthiness of proj_match return, so bool is sufficient for alignment.
func TestMatchProj_ReturnTypeDifference(t *testing.T) {
	b1 := TextBox{Text: "第一章 概述"} // Python priority 1 (chapter)
	b2 := TextBox{Text: "⚫ 要点"}   // Python priority 12 (bullet)

	r1 := MatchProj(b1)
	r2 := MatchProj(b2)

	if !r1 || !r2 {
		t.Errorf("MatchProj should match both heading patterns: got chapter=%v bullet=%v", r1, r2)
	}
	t.Log("NOTE: Go MatchProj returns bool, Python proj_match returns int priority (1-12).")
	t.Log("Python's __filterout_scraps only uses truthiness — bool is sufficient.")
}
