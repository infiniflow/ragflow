...
func (e *Ingestor) Connect() error {
	return nil
}

func (e *Ingestor) Init() error {
	...
	for {
		...
		messages, err = msgQueueEngine.GetMessages(4, true)
		if err != nil {
			common.Error("error consuming message", err)
			continue
		}
		for _, message := range messages {
			common.Info(fmt.Sprintf("Received message: %s", message))
		}
	}
}
