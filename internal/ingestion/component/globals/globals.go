...
// persistKey carries the per-run "persist" flag on the context. When false,
// the pipeline must not perform any persistent side effect (MinIO image
// upload, index insert, pipeline log, ...). It is a control flag owned by the
// executor, distinct from the run-level metadata stored in CanvasState.
type persistKey struct{}

// WithPersist returns a child context carrying the persist flag.
func WithPersist(ctx context.Context, on bool) context.Context {
	return context.WithValue(ctx, persistKey{}, on)
}

// IsPersist reports whether the run should persist its results. Resolution
// order: the context value set by the executor, then CanvasState.Globals
// ("persist"), then a component input map's "persist" key (=="true"),
// defaulting to true so ordinary runs persist. Components read this to gate
// their writes; they never need to know the caller's "debug"/"dry-run"
// vocabulary — only whether this run is allowed to write.
func IsPersist(ctx context.Context, other ...any) bool {
	if v, ok := ctx.Value(persistKey{}).(bool); ok {
		return v
	}
	if st := canvasStateFromContext(ctx); st != nil {
		if v, ok := st.GetGlobal("persist"); ok {
			if s, ok := v.(string); ok && s != "" {
				return s == "true"
			}
		}
	...
	for _, a := range other {
		if inputs, ok := a.(map[string]any); ok {
			if v, ok := inputs["persist"].(string); ok && v != "" {
				return v == "true"
			}
		}
	}
