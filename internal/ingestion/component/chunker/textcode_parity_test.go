...
// TestTextParserGeneralChunkerParity covers the production text&code handoff:
// TextParser emits one normalized JSON unit and GeneralChunker owns delimiter
// splitting plus the final token-budget merge. The Python golden is generated
// by the same TxtParser path used by parser._code; comparison follows the
// content alignment contract and ignores representation-only boundaries and
// whitespace.
func TestTextParserGeneralChunkerParity(t *testing.T) {
	cases := []struct {
		name       string
		samplePath string
		goldenPath string
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if msg, ok := out["_ERROR"].(string); ok && msg != "" {
				t.Fatalf("GeneralChunker returned _ERROR: %s", msg)
			}
			got, ok := out["chunks"].([]map[string]any)
			if !ok {
				t.Fatalf("GeneralChunker chunks has type %T", out["chunks"])
			}
