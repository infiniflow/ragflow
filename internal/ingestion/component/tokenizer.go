...
// HTML or image caption). The concatenation deliberately matches Python's
// doc_tokenize_chunks_with_images contract: no separator is inserted here;
// parser/chunker context values already carry their intended punctuation or
// whitespace.
func contextualChunkText(ck schema.ChunkDoc) string {
	return ck.ContextAbove + ck.Text + ck.ContextBelow
}
