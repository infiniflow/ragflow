...
func chunksFromTokenizerUpstream(in schema.TokenizerFromUpstream) []schema.ChunkDoc {
	...
	// when text, content_with_weight, and both media-context fields are empty.
	...
	// tokenizer. The ContentWithWeight guard preserves the Parser path, whose
	// blocks carry content_with_weight without text; normalizeChunkTextFallback
	// backfills text afterwards, so this guard is required before that pass.
	filtered := raw[:0]
	for _, ck := range raw {
...
func tokenizeChunks(chunks []schema.ChunkDoc, titleStem string, language string) error {
	...
	for i := range chunks {
		...
		} else if t := contextualChunkText(*ck); strings.TrimSpace(t) != "" {
...
func concatFields(ck schema.ChunkDoc, fields []string) string {
	...
	for _, f := range fields {
		switch f {
		case "text":
			b.WriteString(contextualChunkText(ck))
...
// contextualChunkText is the retrieval representation of a chunk's body.
// ContextAbove/ContextBelow are generated by GeneralChunker for media blocks
// and must participate in both full-text tokenization and embeddings, while
// ChunkDoc.Text remains the display/storage payload (for example, the table
// HTML or image caption). Context values normally carry their own punctuation
// or whitespace, matching Python's doc_tokenize_chunks_with_images contract.
// If a producer trims both sides of a boundary, contextualChunkText inserts a
// newline so retrieval and embedding do not fuse adjacent English words.
func contextualChunkText(ck schema.ChunkDoc) string {
	text := ""
	for _, part := range []string{ck.ContextAbove, ck.Text, ck.ContextBelow} {
		if part == "" {
			continue
		}
		if text != "" && !contextBoundaryHasWhitespace(text, part) {
			text += "\n"
		}
		text += part
	}
...
func contextBoundaryHasWhitespace(left, right string) bool {
...
func requiresFullTextTokens(ck schema.ChunkDoc) bool {
	return strings.TrimSpace(ck.Summary) != "" || strings.TrimSpace(contextualChunkText(ck)) != ""
}
