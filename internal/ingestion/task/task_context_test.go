...
func TestLoadTaskContext(t *testing.T) {
	...
	if got.Task.ID != "task-1" {
		t.Errorf("Task.ID = %v, want task-1", got.Task.ID)
	}
	if got.Task.FromPage != 0 {
		t.Errorf("Task.FromPage = %v, want 0", got.Task.FromPage)
	}
	if got.Task.ToPage != 100000 {
		t.Errorf("Task.ToPage = %v, want 100000", got.Task.ToPage)
	}
