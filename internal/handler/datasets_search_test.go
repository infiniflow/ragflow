...
func TestDatasetsHandlerSearchDatasetMapsInvalidSearchID(t *testing.T) {
	db := setupDatasetsSearchHandlerDB(t)
	pushDatasetsSearchHandlerDB(t, db)
	...
	r := setupDatasetSearchRouter(t, "tenant-1")
	req := httptest.NewRequest(http.MethodPost, "/api/v1/datasets/kb-1/search", strings.NewReader(`{"question":"test","search_id":"missing-search"}`))
	req.Header.Set("Content-Type", "application/json")
	w := httptest.NewRecorder()
	r.ServeHTTP(w, req)
	...
	if resp["code"] != float64(102) {
		t.Fatalf("code = %v, want 102", resp["code"])
	}
	if resp["message"] != "Invalid search_id" {
		t.Fatalf("message = %v", resp["message"])
	}
}
