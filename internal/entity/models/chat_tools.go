...
func (cm *ChatModel) ChatWithTools(ctx context.Context, system string, history []Message, chatCfg *ChatConfig) (string, int, error) {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		answer, tokens, err := runToolLoop(cm, h, toolsList, chatCfg, maxRounds)
		if err == nil {
...
func runToolLoop(cm *ChatModel, history []Message, toolsList interface{}, chatCfg *ChatConfig, maxRounds int) (string, int, error) {
	totalTokens := 0
	...
	for round := 0; round <= maxRounds; round++ {
		...
		history = appendToolResults(history, resp.ToolCalls, cm.ToolConfig.ToolCallSession, &totalTokens)
	}
...
func (cm *ChatModel) ChatStreamlyWithTools(ctx context.Context, system string, history []Message, chatCfg *ChatConfig, sender func(*string, *string) error) error {
	tc := cm.ToolConfig
	if tc == nil {
		return fmt.Errorf("ChatStreamlyWithTools called without bound tools")
	}
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		err := runStreamToolLoop(cm, h, toolsList, chatCfg, maxRounds, sender)
		if err == nil {
			return nil
		}
	}
	return fmt.Errorf("ChatStreamlyWithTools failed after %d retries", maxRetries)
}

func runStreamToolLoop(cm *ChatModel, history []Message, toolsList interface{}, chatCfg *ChatConfig, maxRounds int, sender func(*string, *string) error) error {
	for round := 0; round <= maxRounds; round++ {
		cfg := *chatCfg
		cfg.Tools = toolsList
		tcChoice := "auto"
		...
		if len(toolCalls) == 0 {
			return fmt.Errorf("round %d: no content and no tool_calls", round)
		}

		var tokens int
		history = appendToolResults(history, toolCalls, cm.ToolConfig.ToolCallSession, &tokens)
	}
	...
	cfg := *chatCfg
	cfg.Stream = boolPtr(true)
	return cm.ModelDriver.ChatStreamlyWithSender(*cm.ModelName, history, cm.APIConfig, &cfg, sender)
}
...
func appendToolResults(history []Message, toolCalls []map[string]interface{}, session ToolCallSession, tokenCount *int) []Message {
	var mu sync.Mutex
