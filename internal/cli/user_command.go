...
// ShowProvider shows details of a specific provider
// SHOW PROVIDER <name>
func (c *RAGFlowClient) ShowProvider(cmd *Command) (ResponseIf, error) {
	...
	if !ok || code != 0 {
		msg, _ := resJSON["message"].(string)
		return nil, fmt.Errorf("failed to show provider: %s", msg)
	}
	...
	for _, item := range data {
		if provider, ok := item.(map[string]interface{}); ok {
			if name, _ := provider["name"].(string); name == providerName {
				var result CommonResponse
				result.Code = 0
				result.Data = []map[string]interface{}{provider}
				result.Duration = resp.Duration
				return &result, nil
			}
		}
...
// AlterProvider renames a provider
// ALTER PROVIDER <name> NAME <new_name>
func (c *RAGFlowClient) AlterProvider(cmd *Command) (ResponseIf, error) {
	if c.ServerType != "user" {
		return nil, fmt.Errorf("this command is only allowed in USER mode")
	}
	...
	if !ok {
		return nil, fmt.Errorf("new provider name not provided")
	}

	// Build payload for rename operation
	payload := map[string]interface{}{
		"old_llm_factory": providerName,
		"new_llm_factory": newName,
	}

	resp, err := c.HTTPClient.Request("PUT", "/llm/factory", true, "web", nil, payload)
	if err != nil {
		return nil, fmt.Errorf("failed to alter provider: %w", err)
	}

	if resp.StatusCode != 200 {
		return nil, fmt.Errorf("failed to alter provider: HTTP %d, body: %s", resp.StatusCode, string(resp.Body))
	}

	var result SimpleResponse
	if err = json.Unmarshal(resp.Body, &result); err != nil {
		return nil, fmt.Errorf("alter provider failed: invalid JSON (%w)", err)
	}
