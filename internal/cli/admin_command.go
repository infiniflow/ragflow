...
func (c *RAGFlowClient) PingAdmin(cmd *Command) (map[string]interface{}, error) {
	...
	if iterations > 1 {
		...
		result, err := c.HTTPClient.RequestWithIterations("GET", "/admin/ping", false, "web", nil, nil, iterations)
		if err != nil {
			return nil, err
		}
		return result, nil
	...
	if result.Code == 0 && result.Message == "pong" {
