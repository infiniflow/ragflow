...
def bind_dynamic_tools(tools_slot) -> list:
    ...
    Binds on the innermost ``Base`` instance (see :func:`_base_chat_mdl`), NOT on
    the ``LLMBundle`` proxy (whose ``bind_tools`` is legacy-style with a required
    ``toolcall_session`` positional arg and an ``is_tools`` gate). The decorator
    style ``bind_tools(tools=[...])`` auto-derives the schema + a
    ``FunctionToolSession``, which the built-in tool loop reads via ``self.tools``
    / ``self.toolcall_session``.

    The schema ``function.name`` for each tool is rewritten to the clean public
    name (``grep_chunks`` / ``search_chunks`` / ``list_chunks`` / ``think`` /
    ``todo_write`` / ``calculate``, plus ``navigate_tree`` and ``navigate_structure``
    in high/ultra modes) BEFORE binding, so the schema the model sees matches the
    registration key and the names referenced by the prompt.

    Mode awareness: when ``tools_slot.thinking_mode`` is ``high`` or ``ultra``,
    the knowledge-compilation tools (``navigate_tree`` for tree routing,
    ``navigate_structure`` for reading a document's compiled structure) are bound
    in addition to the default five. Returns the bound ``@tool`` callables (for
    tests / introspection).
    """
    ...
    if mode in _COMPILED_TOOL_MODES:
        # The compiled navigation tools live in harness/tools/navigation.py;
        # import them lazily so the search module has no hard module-load
        # dependency on the navigation tool bindings.
        from rag.advanced_rag.harness.tools.navigation import (
            _navigate_structure_impl,
            _navigate_tree_impl,
        )
    ...
    mdl = _base_chat_mdl(tools_slot)
    if mdl is None:
        _LOG.error("[dynamic] no usable chat model to bind tools onto")
        return callables
    try:
        mdl.bind_tools(tools=callables)
        mdl.is_tools = True
    except Exception:
        _LOG.exception("[dynamic] bind_tools on base model failed")
    return callables
