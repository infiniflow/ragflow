...
def _field_lines(field_map):
    return "\n".join(
        f"  - {field} ({descriptor}): chunk_data #>> {_json_path_literal(field, descriptor)}"
...
def build_source_reference(table, kb_ids):
    ...
    return {
        "chunks": chunks,
        "doc_aggs": [
            {"doc_id": doc_id, "doc_name": values["doc_name"], "count": values["count"]}
            for doc_id, values in doc_aggs.items()
        ],
    }
...
def complete_reference_kb_ids(result, table_name, kb_ids, validator, sql_retrieval):
    ...
    try:
        ...
        if doc_idx is None or kb_idx is None:
            return
        mapping = {
            row[doc_idx]: row[kb_idx]
            for row in lookup_tbl.get("rows", [])
            if len(row) > max(doc_idx, kb_idx)
        }
        for chunk in chunks:
