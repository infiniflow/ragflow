...
def _parse_floatvector(value, cur):
    ...
    Mirrors what the pyobvector driver does for OceanBase, so chunk vectors
    reach the retrieval layer (Dealer.rerank / fetch_chunk_vectors) already
    typed as lists and the string sniffing there stays a fallback only.
    """
    if not value:
        return None
    if isinstance(value, bytes):
        value = value.decode()
    return [get_float(x) for x in value.strip("[]").split(",")]


def _register_vector_typecaster(dbapi_connection, connection_record):
    """Resolve the cluster-specific floatvector OID and register the caster.

    floatvector is an extension type whose OID is assigned per cluster, so it
    must be looked up in pg_type on connect instead of hard-coded. Runs once
    per pooled DBAPI connection via the SQLAlchemy "connect" event.
    """
    try:
        with dbapi_connection.cursor() as cur:
            cur.execute("SELECT oid FROM pg_type WHERE typname = 'floatvector'")
            oids = [row[0] for row in cur.fetchall()]
    except Exception as e:
        logger.warning(f"Failed to look up floatvector OID, vectors will arrive as text: {e}")
        return
    for oid in oids:
        register_type(new_type((oid,), "floatvector", _parse_floatvector), dbapi_connection)
...
_MAPPING_SCHEMA_CACHE = None


def _chunk_mapping_schema() -> dict:
    global _MAPPING_SCHEMA_CACHE
    if _MAPPING_SCHEMA_CACHE is None:
        ...
        try:
            ...
            _MAPPING_SCHEMA_CACHE = {}
    return _MAPPING_SCHEMA_CACHE


def ensure_table_columns(conn: psycopg2.extensions.connection, table_name: str, table_instance: list, field_names) -> list | None:
    """Add columns for fields the table doesn't have yet (idempotent).

    Chunk tables carry a static schema from vastbase_mapping.json, but upstream
    can start writing new chunk fields (e.g. compile_kwd from the knowledge
    compile features) at any time — ES's dynamic mapping absorbs that silently.
    Mirror that behavior: when documents carry fields the table lacks, ALTER
    TABLE ADD COLUMN using the mapping's declared type (varchar fallback for
    unknown fields, floatvector(N) for q_*_vec). Returns a refreshed
    table_instance, or None when nothing changed.
    """
    existing = {n for n, *_ in table_instance}
    missing = sorted({f for f in field_names if f and f not in existing})
    if not missing:
        return None
    schema = _chunk_mapping_schema()
    for f in missing:
        m = re.match(r"q_(\d+)_vec$", f)
        if m:
            ftype, fdefault = f"floatvector({m.group(1)})", None
        else:
            info = schema.get(f, {})
            ftype, fdefault = info.get("type", "varchar(256)"), info.get("default", "")
        try:
            with conn.cursor() as cur:
                cur.execute(
                    sql.SQL("ALTER TABLE {t} ADD COLUMN {c} {ty} DEFAULT {d}").format(
                        t=sql.Identifier(table_name),
                        c=sql.Identifier(f),
                        ty=sql.SQL(ftype),
                        d=sql.Literal(fdefault),
                    )
                )
            conn.commit()
        except Exception as e:
            ...
            if "already exists" not in str(e).lower():
                raise
            # A concurrent worker added it first; keep going.
    logger.info(f"Vastbase added missing columns {missing} to table {table_name}")
    return get_table_instance(conn, table_name)
...
def equivalent_condition_to_str(condition: dict, table_instance=None) -> str | None:
    ...
    def exists(cln):
        nonlocal clmns
        if cln not in clmns:
            # ES dynamic-mapping parity: a field this table never stored
            # exists on no row.
            return "1=0"
        if _text_column(clmns[cln][0]):
            # Unset text columns hold DEFAULT '' (not NULL), so '' plays the
            # role of ES's "field absent".
            return f"({quote_ident(cln)} IS NOT NULL AND {quote_ident(cln)} <> '')"
    ...
    for k, v in condition.items():
        ...
        if field_keyword(k):
            pass
        elif k not in ("must_not", "exists") and clmns and k not in clmns:
            # Filtering on a column this table doesn't have: no row can
            # match, like an unmapped ES field.
            cond.append("1=0")
            continue
            ...
            if isinstance(v, dict):
                for kk, vv in v.items():
                    if kk == "exists":
                        if vv not in clmns:
                            # Column absent => NULL on every row, so the
                            # must_not clause holds for all of them.
                            continue
                        if _text_column(clmns[vv][0]):
                            # Text columns default to '' rather than NULL, so
                            # "field absent" means NULL or empty string.
                            cond.append(f"({quote_ident(vv)} IS NULL OR {quote_ident(vv)} = '')")
                        else:
...
class VBConnection(DocStoreConnection):
    def __init__(self):
        ...
        url = f"postgresql+psycopg2://{quote_plus(vb_user)}:{quote_plus(vb_password)}@{vb_host}:{vb_port}/{self.dbName}"
        ...
        for _ in range(24):
            try:
                ...
                # Register the floatvector typecaster on every pooled
                # DBAPI connection before any query can run on it.
                event.listen(engine, "connect", _register_vector_typecaster)
                # Test connection
        ...
        with self.get_conn() as vb_conn:
            try:
                with vb_conn.cursor() as cur:
                    ...
                    cur.fetchone()
                res = {"type": "vastbase", "status": "green", "error": ""}
            except Exception as e:
                res = {"type": "vastbase", "status": "red", "error": str(e)}
            return res
    ...
    def create_idx(self, index_name: str, dataset_id: str, vector_size: int, parser_id: str = None):
        ...
        with self.get_conn() as vb_conn:
            ...
            for field_name, field_info in schema.items():
                ...
                columns.append(
                    sql.SQL("{field_name} {field_type} DEFAULT {field_default}").format(field_name=sql.Identifier(field_name), field_type=sql.SQL(field_type), field_default=sql.Literal(field_default))
                )

            # Add vector field
            columns.append(sql.SQL("{vector_name} floatvector({vectorSize})").format(vector_name=sql.Identifier(vector_name), vectorSize=sql.SQL(str(vector_size))))
            ...
            """).format(table_name=sql.Identifier(table_name), columns=sql.SQL(", ").join(columns))
            ...
            with vb_conn.cursor() as cur:
                ...
                """).format(index_name=sql.Identifier(f"q_vec_idx_{table_name}"), table_name=sql.Identifier(table_name), vector_name=sql.Identifier(vector_name))
                ...
                text_idx_fields = ["title_tks", "title_sm_tks", "important_kwd", "important_tks", "question_tks", "content_ltks", "content_sm_ltks"]

                if db_compatibility == "PG":
                    # PG-compatible: single GIN index with to_tsvector.
                    try:
                        field_list = sql.SQL(", ").join(sql.SQL("to_tsvector('cn_tokenizer', {})").format(sql.Identifier(f)) for f in text_idx_fields)
                        pg_fts_sql = sql.SQL("""
                            CREATE INDEX IF NOT EXISTS {index_name}
                            ON {table_name} USING gin({field_list})
                        """).format(index_name=sql.Identifier(f"text_gin_idx_{table_name}"), table_name=sql.Identifier(table_name), field_list=field_list)
                        logging.debug(f"VASTBASE create PG fulltext index SQL: {pg_fts_sql.as_string(vb_conn)}")
                        cur.execute(pg_fts_sql)
                        vb_conn.commit()
                        logger.info(f"VASTBASE created PG fulltext index for table {table_name}")
                    except Exception as e:
                        logging.warning(f"VASTBASE PG fulltext index failed, vector search will work without it: {e}")
                        vb_conn.rollback()
                    ...
                    for f in text_idx_fields:
                        try:
                            ...
                            """).format(table_name=sql.Identifier(table_name), index_name=sql.Identifier(f"{f}_fulltext_idx_{table_name}"), field_name=sql.Identifier(f))
                            cur.execute(mysql_fts_sql)
                            vb_conn.commit()
                        except Exception as e2:
                            if "already exists" not in str(e2):
                                logging.warning(f"VASTBASE failed to create fulltext index for {f}: {e2}, vector search will work without it")
                            vb_conn.rollback()
                    logger.info(f"VASTBASE created MySQL fulltext indexes for table {table_name}")
                else:
                    logger.warning(f"VASTBASE unknown dbcompatibility '{db_compatibility}', skipping fulltext index creation")
    ...
    def delete_idx(self, index_name: str, dataset_id: str):
        ...
        with self.get_conn() as vb_conn:
            with vb_conn.cursor() as cur:
                drop_index_sql = sql.SQL("DROP TABLE IF EXISTS {table_name}").format(table_name=sql.Identifier(table_name))
    ...
    def create_doc_meta_idx(self, index_name: str):
        ...
        with self.get_conn() as vb_conn:
            if get_table_exists(vb_conn, table_name):
                return True
            fp_mapping = os.path.join(get_project_base_directory(), "conf", "doc_meta_vastbase_mapping.json")
            if not os.path.exists(fp_mapping):
    ...
    def search(
                ...
                if isinstance(matchExpr, MatchTextExpr):
                    ...
                    if fields:
                        ...
                        for field_name, field_weight in fields:
                            fulltext_ft_parts.append(
                                sql.SQL("{column} @~@ {matching_text}").format(
                                    column=sql.Identifier(field_name), matching_text=sql.Literal(f"{matching_text} @<PARAM:MINIMUM_SHOULD_MATCH={minimum_should_match} PARAM:BOOST={field_weight}>@")
                                )
                            )
                        ...
                        if len(fulltext_ft_parts) == 1:
                            ...
                            if filter_cond:
                                filter_fulltext = sql.SQL("({filter_cond}) AND ({filter_fulltext})").format(filter_cond=sql.SQL(filter_cond), filter_fulltext=filter_fulltext)
                ...
                for knowledgebaseId in knowledgebase_ids:
                    ...
                    if len(match_expressions) > 0:
                        for matchExpr in match_expressions:
                            if isinstance(matchExpr, MatchTextExpr):
                                ...
                                if len(fulltext_ft_parts) > 1:
                                    ...
                                    ORDER BY bm25_score DESC
                                    LIMIT {limit})
                                    """).format(select_fields=select_fields_sql, table_name=sql.Identifier(table_name), filter_fulltext=filter_fulltext, limit=sql.Literal(fulltext_limit))
                        ...
                        if filter_cond and len(filter_cond) > 0:
                            ...
                            FROM {table_name}
                            WHERE {filter_clause}
                            """).format(select_fields=select_fields_sql, table_name=sql.Identifier(table_name), filter_clause=sql.SQL(filter_cond))
                    if sql_expr is None:
                        logger.warning(
                    ...
                    if order_by.fields:
                        ...
                        """).format(
                            sql_query=sql_query,
                            order_clause=sql.SQL(", ").join([sql.SQL("{field} {sort}").format(field=sql.Identifier(field), sort=sql.SQL(sort)) for field, sort in order_by_expr_list]),
                        )
                    sql_query = sql.SQL("""
                    {sql_query}
                    LIMIT {limit} OFFSET {offset}""").format(sql_query=sql_query, limit=sql.Literal(limit), offset=sql.Literal(offset))
        ...
        if match_expressions:
            ...
            score_col = (
                score_column
                if score_column in res.columns
                else (
                    "SIMILARITY"
                    if "SIMILARITY" in res.columns
                    else ("SCORE" if "SCORE" in res.columns else ("score" if "score" in res.columns else ("similarity" if "similarity" in res.columns else None)))
                )
            )
    ...
    def get(self, data_id: str, index_name: str, dataset_ids: list[str]) -> dict | None:
        with self.get_conn() as vb_conn:
            ...
            for knowledgebaseId in dataset_ids:
                ...
                table_exists = get_table_exists(vb_conn, table_name)
                if not table_exists:
                    logger.warning(f"Table not found: {table_name}, this knowledge base isn't created in Vastbase. Maybe it is created in other document engine.")
                    continue
                with vb_conn.cursor() as cur:
                    cur.execute(sql.SQL("SELECT * FROM {table_name} WHERE id = %s").format(table_name=sql.Identifier(table_name)), (data_id,))
                    column_names = [desc[0] for desc in cur.description]
                    rows = cur.fetchall()
                kb_res = pd.DataFrame(rows, columns=column_names)
                df_list.append(kb_res)
    ...
    def insert(self, documents: list[dict], index_name: str, dataset_id: str = None, refresh: str | bool = "wait_for") -> list[str]:
        # `refresh` is accepted for DocStoreConnection interface parity (ES
        # engines expose near-real-time search and may wait for a refresh);
        # Vastbase commits per statement, so inserted rows are visible at once
        # and the flag is intentionally ignored.
        with self.get_conn() as vb_conn:
            if index_name.startswith("ragflow_doc_meta_"):
                table_name = index_name
            else:
            ...
            # Older tables may predate newly-introduced chunk fields.
            refreshed = ensure_table_columns(vb_conn, table_name, table_instance, (k for d in documents for k in d.keys()))
            if refreshed is not None:
                table_instance = refreshed
            ...
            with vb_conn.cursor() as cur:
                ...
                insert_sql = sql.SQL("INSERT INTO {table_name} ({column_names}) VALUES %s").format(
                    table_name=sql.Identifier(table_name), column_names=sql.SQL(", ").join([sql.Identifier(col) for col in column_names])
                )
        ...
        with self.get_conn() as vb_conn:
            ...
            with vb_conn.cursor() as cur:
                if removeValue:
                    ...
                    cur.execute(
                        sql.SQL("SELECT {columns} FROM {table_name} WHERE {filter_clause}").format(
                            columns=sql.SQL(", ").join([sql.Identifier(col) for col in col_to_remove]), table_name=sql.Identifier(table_name), filter_clause=sql.SQL(filter)
                        )
                    )
                    column_names = [desc[0] for desc in cur.description]
                    rows = cur.fetchall()
                ...
                for update_kv, ids in remove_opt.items():
                    ...
                    cur.execute(
                        sql.SQL("UPDATE {table_name} SET {k}=%s WHERE {filter_clause} AND id in %s").format(table_name=sql.Identifier(table_name), k=sql.Identifier(k), filter_clause=sql.SQL(filter)),
                        ("###".join(v), tuple(ids)),
                    )
    ...
    def delete(self, condition: dict, index_name: str, dataset_id: str) -> int:
        with self.get_conn() as vb_conn:
            ...
            if not table_instance:
                ...
                return 0
            filter = equivalent_condition_to_str(condition, table_instance)
            delete_sql = sql.SQL("DELETE FROM {table_name} WHERE {filter_clause}").format(table_name=sql.Identifier(table_name), filter_clause=sql.SQL(filter))
    ...
    def get_fields(self, res: tuple[pd.DataFrame, int] | pd.DataFrame, fields: list[str]) -> dict[str, dict]:
        ...
        for column in res2.columns:
            ...
            if field_keyword(k):
                ...
                res2[column] = res2[column].apply(lambda v: json.loads(v) if isinstance(v, str) and v.strip().startswith("{") else (v or {}))
            else:
                pass
