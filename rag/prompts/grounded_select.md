...
```json
{
    "claims": [
        ...
        },
        {
            ...
            "missing_information": [
                {"what": "the patient's allergic reactions or adverse events during the stay", "search_hint": "allergy rash adverse events discharge"}
            ]
        }
    ]
}
...
Missing-information analysis (forward gap, Google Agentic RAG style):
For each claim, `missing_information` lists what the CLAIM still needs in order to be
fully answerable but is ABSENT from the current evidence. This is distinct from
`ungrounded_assertions`:
- `ungrounded_assertions` = the draft said something the evidence does NOT support
  (over-claim / hallucination) -> the assertion should be dropped or corrected.
- `missing_information` = the claim's answer REQUIRES a fact/entity/value that the
  evidence simply does not contain -> the search loop should go retrieve it.
Populate `missing_information` when the evidence, even if the report is fully
grounded, does NOT cover a part the question asks for. Examples:
- the question asks for a specific property (age / year / employer / distance /
  count) and the evidence has the entity but not that property;
- a disambiguation is needed (evidence has a same-named but wrong entity, e.g. the
  ice-hockey "Ron Hutchinson" when the question means the football player);
- the evidence only covers part of an enumeration (some members of a list present,
  others absent).
Give a `search_hint`: a concrete, searchable query (keywords / qualifiers) that would
retrieve the missing fact. Leave `missing_information` empty `[]` when the evidence is
...
5. `missing_information` must always be present (empty list when the claim is fully covered). Use it to drive a targeted re-search of the absent fact, never to re-state `ungrounded_assertions`.
