...
Execute this single direction. Return EXACTLY ONE of:

1) TOOL CALL MODE — call `retrieve` (the only bound tool) with 1-3 corpus queries.
   Results arrive in the next turn.

2) STATE PATCH MODE — when you have partial updates, emit:
<state>
{"new_states": [
...
3) FINAL ANSWER MODE — only when ALL slots can be filled consistently:
<answer>
