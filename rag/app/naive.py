            ...
            try:
                ...
                for idx, (section_text, _) in enumerate(sections):
                    ...
                    if images:
                        ...
                        markdown_parser_vision = VisionFigureParser(vision_model=vision_model, figures_data= [((combined_image, ["markdown image"]), [(0, 0, 0, 0, 0)])], **kwargs)
                        boosted_figures = markdown_parser_vision(callback=callback)
                        sections[idx] = (section_text + "\n\n" + "\n\n".join([fig[0][1] for fig in boosted_figures]), sections[idx][1])
                    else:
                        section_images.append(None)
            except Exception as e:
                logging.warning(f"Visual model error: {e}. Skipping image parsing enhancement.")
