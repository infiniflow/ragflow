...
class Excel(ExcelParser):
    def __call__(self, fnm, binary=None, from_page=0, to_page=MAXIMUM_TASK_PAGE_NUMBER, callback=None, lang="English", **kwargs):
        if not binary:
        ...
        for sheet_name in wb.sheetnames:
            ...
            pending_cell_images = []
            if images:
                image_descriptions = vision_figure_parser_figure_xlsx_wrapper(images=images, callback=callback, lang=lang, **kwargs)
...
def chunk(filename, binary=None, from_page=0, to_page=MAXIMUM_TASK_PAGE_NUMBER, lang="Chinese", callback=None, **kwargs):
    ...
    if re.search(r"\.xlsx?$", filename, re.IGNORECASE):
        ...
        excel_parser = Excel()
        dfs, tbls = excel_parser(filename, binary, from_page=from_page, to_page=to_page, callback=callback, lang=lang, **kwargs)
