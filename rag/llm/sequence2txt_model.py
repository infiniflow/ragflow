...
class Ai302Seq2txt(Base):
    ...
    def transcription(self, audio, language=None, translate=False, processing_type="none", output="raw"):
        """
        Transcribe audio using 302.AI WhisperX API
        
        Args:
            audio: Audio file (bytes, file path, or BytesIO)
            language: Language code for transcription (auto-detect if None)
            translate: Whether to translate the content
            processing_type: Either 'diarize', 'align', or 'none'
            output: Either 'raw' or 'subtitles'
        """
        try:
            # Prepare audio file
            if isinstance(audio, str):
                ...
                # If audio is a file-like object (BytesIO)
                audio_data = audio.read()
                audio_filename = "audio.wav"
            else:
                raise TypeError("Audio must be a file path, bytes, or file-like object")

            # Prepare form data
            files = {
                "audio_input": (audio_filename, audio_data, "audio/wav")
            }
            ...
            if isinstance(result, dict):
                ...
                if "text" in result:
                    ...
                    # If segments are present, concatenate all text
                    transcription_text = " ".join([segment.get("text", "") for segment in result["segments"]])
                elif "transcription" in result:
                    transcription_text = result["transcription"]
                else:
                    # If the result structure is different, try to extract text
            ...
            return f"**ERROR**: Request failed - {str(e)}", 0
        except Exception as e:
            return f"**ERROR**: {str(e)}", 0
