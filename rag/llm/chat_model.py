...
class YAPIChat(Base):
    """Y-API OpenAI-compatible chat adapter.

    Y-API is a hosted multi-model gateway speaking the OpenAI wire format, so
    the standard OpenAI client path covers every call. The base URL is
    tenant-configurable because Y-API is also reachable behind custom domains.
    """
