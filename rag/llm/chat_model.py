...
class HuggingFaceChat(Base):
    def __init__(self, key, model_name, **kwargs):
        self.base_url = kwargs.get("base_url", "https://api-inference.huggingface.co/models/")
        self.model_name = model_name
        self.headers = {
    ...
    def chat(self, system, history, gen_conf):
        ...
        try:
            ...
            if "max_tokens" in gen_conf:
                options["max_length"] = gen_conf["max_tokens"]
            if "top_p" in gen_conf:
            ...
            response = requests.post(f"{self.base_url}{self.model_name}", headers=self.headers, json=data)
    ...
    def chat_streamly(self, system, history, gen_conf):
        ...
        ans = ""
        try:
            response = requests.post(f"{self.base_url}{self.model_name}", headers=self.headers, json={
                "inputs": {
                    "messages": history,
                    **options
                },
