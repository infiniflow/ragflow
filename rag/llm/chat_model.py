...
class LiteLLMBase(ABC):
    ...
    def _construct_completion_args(self, history, stream: bool, tools: bool, **kwargs):
        ...
        if self.provider in FACTORY_DEFAULT_BASE_URL:
            ...
            # Parse the Bedrock key via the shared helper. On non-JSON input
            # (e.g. a plain AWS access key) the helper raises a clear
            # ModelException with the required schema; without this guard a
            # raw ``json.loads(self.api_key)`` would surface a JSONDecodeError
            # from inside rag/llm internals (see #17373).
