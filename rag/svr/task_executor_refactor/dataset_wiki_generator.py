...
from rag.advanced_rag.knowlege_compile.wiki import (
    ...
    _wiki_has_active_map_state_marker,
...
async def run_wiki_incremental(
    ...
    # Freeze the legacy one-row-per-chunk MAP baseline before writing any new
    # historical version.  If this build later fails, the next attempt still
    # compares against the last successful legacy Wiki instead of selecting an
    # arbitrary hash from the now multi-version cache.
    if not await _wiki_has_active_map_state_marker(ctx.tenant_id, ctx.kb_id):
        await _wiki_commit_active_map_state(ctx.tenant_id, ctx.kb_id, previous_chunk_state)
