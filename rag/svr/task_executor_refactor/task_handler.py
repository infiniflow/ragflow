...
_WIKI_GRAPH_MAX_CHUNK_IDS_PER_NODE = 64

# Title + comments stamped on every artifact_commit row produced by
# _run_wiki's regeneration path (as opposed to the dialog-edit
# path, which carries the user's own title/comments). Centralized here
# so the wording is editable in one place.
_WIKI_REGEN_COMMIT_TITLE = "Regenerated by artifact compilation"
_WIKI_REGEN_COMMIT_COMMENTS_TEMPLATE = "Auto-update via _run_wiki (action={action})"
...
class TaskHandler:
    ...
    async def handle(self) -> None:
        ...
        with embedding_model:
            ...
            if task_type == "graphrag":
                ...
                await self._run_wiki(embedding_model)
    ...
    async def _run_wiki(self, embedding_model):
        ...
        # built-in ``WIKI_TEMPLATE_EXAMPLE``).
        ...
        for i, (doc, template_id) in enumerate(eligible):
            ...
            async for batch in self._load_chunks_for_doc(
                ...
                batch_size=_WIKI_MAP_BATCH_CHUNKS,
            ):
                saw_any = True
                batch_no += 1
                try:
        ...
        try:
            await self._persist_wiki_pages_to_es(ctx, pages or [], embedding_model)
        except Exception:
        ...
        try:
            await self._persist_wiki_page_graph_to_es(ctx, pages or [])
        except Exception:
    ...
    async def _persist_wiki_pages_to_es(
        ...
        from rag.nlp import rag_tokenizer
        from common.doc_store.doc_store_base import OrderByExpr
        from api.db.services.artifact_commit_service import ArtifactCommitService as WikiCommitService
        ...
        try:
            ...
            logging.exception("wiki_persist: summary embedding batch failed for kb=%s", kb_id_str)
            return
        try:
        ...
        for page in pages:
            ...
            try:
                WikiCommitService.record_edit(
                    ...
                    title=_WIKI_REGEN_COMMIT_TITLE,
                    comments=_WIKI_REGEN_COMMIT_COMMENTS_TEMPLATE.format(action=action),
    ...
    def _build_wiki_page_graph(
        ...
        get_wiki_graph``) now reads ``artifact_entity`` /
        ...
        for p in pages or []:
            ...
            for cid in raw_chunk_ids:
                if isinstance(cid, str) and cid and cid not in seen_chunk_ids:
                    ...
                    if len(seen_chunk_ids) >= _WIKI_GRAPH_MAX_CHUNK_IDS_PER_NODE:
                        break
    ...
    async def _persist_wiki_page_graph_to_es(
        ...
        ``dataset_api_service.get_wiki_graph`` reads exactly these
        ...
        """
        kb_id_str = str(ctx.kb_id)
        entity_rows, relation_rows = self._build_wiki_page_graph(
            pages or [], kb_id_str,
        )
