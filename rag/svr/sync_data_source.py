...
from common.data_source.highspot_connector import HighspotConnector
...
class Highspot(SyncBase):
    SOURCE_NAME: str = FileSource.HIGHSPOT
    ...
    async def _generate(self, task: dict):
        ...
        self.connector = HighspotConnector(
            base_url=self.conf.get("base_url") or None,
            spot_ids=_as_list(self.conf.get("spot_ids")),
            batch_size=batch_size,
            include_files=bool(self.conf.get("include_files", True)),
        )
        self.connector.load_credentials(self.conf["credentials"])
        # Fail fast on invalid credentials / Spots before crawling,
        ...
        document_generator = self.connector.poll_source(start_time, end_time)
        self.log_connection("Highspot", self.connector.base_url, task)
        return document_generator
