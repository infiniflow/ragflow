...
from common.data_source.looker_connector import LookerConnector
...
class Looker(SyncBase):
    SOURCE_NAME: str = FileSource.LOOKER

    async def _generate(self, task: dict):
        base_url = (self.conf.get("base_url") or "").strip()
        if not base_url:
            raise ValueError("Looker base URL is required.")
        ...
        self.connector = LookerConnector(
            base_url=base_url,
            include_dashboards=bool(self.conf.get("include_dashboards", True)),
            include_looks=bool(self.conf.get("include_looks", True)),
            include_exports=bool(self.conf.get("include_exports", False)),
            batch_size=batch_size,
        )
        self.connector.load_credentials(self.conf["credentials"])
        # Fail fast on invalid base URL / credentials before crawling,
        ...
        document_generator = self.connector.poll_source(start_time, end_time)
        self.log_connection("Looker", base_url, task)
        return document_generator
