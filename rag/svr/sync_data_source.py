...
from common.data_source.lumapps_connector import LumAppsConnector
...
class LumApps(SyncBase):
    SOURCE_NAME: str = FileSource.LUMAPPS

    async def _generate(self, task: dict):
        base_url = (self.conf.get("base_url") or "").strip()
        if not base_url:
            raise ValueError("LumApps base URL is required.")
        ...
        self.connector = LumAppsConnector(
            base_url=base_url,
            community_ids=_as_list(self.conf.get("community_ids")),
            content_types=_as_list(self.conf.get("content_types")),
            lang=self.conf.get("lang") or None,
            token_url=self.conf.get("token_url") or None,
            include_attachments=bool(self.conf.get("include_attachments", False)),
            batch_size=batch_size,
        )
        self.connector.load_credentials(self.conf["credentials"])
        # Fail fast on invalid base URL / token before crawling, regardless
        # of how the config was persisted.
        self.connector.validate_connector_settings()
        ...
        document_generator = self.connector.poll_source(start_time, end_time)
        self.log_connection("LumApps", base_url, task)
        return document_generator
