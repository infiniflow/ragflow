...
async def build_chunks(task, progress_callback):
  ...
  try:
    async with chunk_limiter:
      cks = await trio.to_thread.run_sync(lambda: chunker.chunk(
          task["name"], binary=binary, from_page=task["from_page"], to_page=task["to_page"], lang=task["language"],
          callback=progress_callback, kb_id=task["kb_id"], parser_config=task["parser_config"], tenant_id=task[
              "tenant_id"]))
  ...
  async def upload_to_minio(document, chunk):
    try:
      ...
      d.update(chunk)
      d["id"] = xxhash.xxh64((chunk["content_with_weight"] + str(d["doc_id"])).encode("utf-8",
  ...
  if task["parser_config"].get("auto_questions", 0):
    ...
    for d in docs:
      ...
      if task_canceled:
        ...
        return
      if settings.retriever.tag_content(
          tenant_id, kb_ids, d, all_tags, topn_tags=topn_tags, S=S) and len(d[TAG_FLD]) > 0:
...
def build_TOC(task, docs, progress_callback):
  ...
  docs = sorted(
      docs,
      key=lambda d: (d.get("page_num_int", 0)[0]
                     if isinstance(d.get("page_num_int", 0), list) else d.get("page_num_int", 0), d.get("top_int", 0)[0]
                     if isinstance(d.get("top_int", 0), list) else d.get("top_int", 0)))
...
async def run_dataflow(task: dict):
  ...
  if not chunks:
    PipelineOperationLogService.create(
        document_id=doc_id, pipeline_id=dataflow_id, task_type=PipelineTaskType.PARSE, dsl=str(pipeline))
    return
  ...
  logging.info("[Done], chunks({}), token({}), elapsed:{:.2f}".format(
...
async def run_raptor_for_kb(row, kb_parser_config, chat_mdl, embd_mdl, vector_size, callback=None, doc_ids=[]):
  ...
  for doc_id in doc_ids:
    for d in settings.retriever.chunk_list(
        doc_id, row["tenant_id"], [str(row["kb_id"])], fields=["content_with_weight", vctr_nm], sort_by_position=True):
  ...
  for b in range(0, len(chunks), DOC_BULK_SIZE):
    ...
    try:
      ...
      doc_store_result = await trio.to_thread.run_sync(
          lambda: settings.docStoreConn.delete({"id": chunk_ids}, search.index_name(task_tenant_id), task_dataset_id))
      async with trio.open_nursery() as nursery:
...
async def do_handle_task(task):
  ...
  logging.info("Indexing doc({}), page({}-{}), chunks({}), elapsed: {:.2f}".format(task_document_name,
                                                                                   task_from_page,
                                                                                   task_to_page,
                                                                                   len(chunks),
                                                                                   timer() - start_ts))
  ...
  logging.info("Chunk doc({}), page({}-{}), chunks({}), token({}), elapsed:{:.2f}".format(
