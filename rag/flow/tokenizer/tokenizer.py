...
class Tokenizer(ProcessBase):
    ...
    async def _embedding(self, name, chunks):
        ...
        cnts_ = np.array([])
        for i in range(0, len(texts), app_config.rag.embedding_batch_size):
            async with embed_limiter:
                vts, c = await asyncio.to_thread(batch_encode,texts[i : i + app_config.rag.embedding_batch_size],)
            if len(cnts_) == 0:
                cnts_ = vts
            else:
            ...
            token_count += c
            if i % 33 == 32:
                self.callback(i * 1.0 / len(texts) / parts / app_config.rag.embedding_batch_size + 0.5 * (parts - 1))
