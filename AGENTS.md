...
- T0 provenance registries and their capture command are documented in [the inventory report](docs/develop/t0-provenance-report-ru.md). Refresh affected records for the actual working snapshot; the report does not certify behavior. The first [Python observation tool](docs/develop/t2-python-analysis-ru.md) reports imports, parent initialization, unresolved dynamic loading and complexity signals. It does not enforce layers or identify removable code. Full architecture/dead-code runners and CI gates remain future work; do not claim they pass or create guessed upstream SHAs or empty baselines.
