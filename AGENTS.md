...
- T0 provenance registries and their capture command are documented in [the inventory report](docs/develop/t0-provenance-report-ru.md). Refresh affected records for the actual working snapshot; the report does not certify behavior. The [Python T2 tools](docs/develop/t2-python-analysis-ru.md) report imports, imported symbols, parent initialization, direct reverse consumers, unresolved dynamic loading and complexity signals; the report-only runner currently evaluates configured ARC-01/02 static slices, a scoped ARC-03 explicit-cycle check, isolated domain-import and HTTP-registration probes for Business Documents/OpenMetadata, exact Canvas DSL contracts for the locally owned OpenMetadata component and changed Retrieval tool registries, focused contracts for two Business Documents production adapters, and exact locally owned frontend route entrypoints. The separate DEAD-01 scanner performs only a static `scan/plan` for explicitly configured owned profiles; `OBSERVED`, zero candidates and `STATIC_CANDIDATE` never confirm deletion, and apply mode does not exist. The [Go build-profile planner](docs/develop/t2-go-build-profiles-ru.md) maps every changed `.go` in T0 to an owned `build.sh` build or package-test command; `READY/NOT_EVALUATED` is only a complete plan, while missing Linux/native tools, declared artifacts or structured test evidence is `INCOMPLETE`. These tools do not verify historical saved DSL or provide a full architecture/dead-code verdict. Other registrations/adapters, broad TypeScript/Go/cycle/dead-code analysis and CI gates remain future work; do not claim they pass or create guessed upstream SHAs or empty baselines.
