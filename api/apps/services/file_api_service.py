...
async def rename_file(uid: str, file_id: str, name: str):
    """
    Rename a file.
    ...
    if not check_file_team_permission(file, uid):
        return False, "No authorization."
    if file.type != FileType.FOLDER.value \
        and pathlib.Path(name.lower()).suffix != pathlib.Path(file.name.lower()).suffix:
        return False, "The extension of file can't be changed"

    for f in FileService.query(name=name, pf_id=file.parent_id):
        if f.name == name:
            return False, "Duplicated file name in the same folder."

    if not FileService.update_by_id(file_id, {"name": name}):
        return False, "Database error (File rename)!"

    informs = File2DocumentService.get_by_file_id(file_id)
    if informs:
        if not DocumentService.update_by_id(informs[0].document_id, {"name": name}):
            return False, "Database error (Document rename)!"

    return True, True


async def move_files(uid: str, src_file_ids: list, dest_file_id: str):
    """
    Move files to a destination folder with storage-layer move.

    :param uid: user ID
    :param src_file_ids: list of source file IDs
    :param dest_file_id: destination folder ID
    :return: (success, result) or (success, error_message)
    """
    ...
    def _move_entry_recursive(source_file_entry, dest_folder_entry):
        if source_file_entry.type == FileType.FOLDER.value:
            existing_folder = FileService.query(name=source_file_entry.name, parent_id=dest_folder_entry.id)
            if existing_folder:
        ...
        try:
            settings.STORAGE_IMPL.move(old_parent_id, old_location, dest_folder_entry.id, new_location)
        except Exception as storage_err:
            raise RuntimeError(f"Move file failed at storage layer: {str(storage_err)}")

        FileService.update_by_id(
            source_file_entry.id,
            {"parent_id": dest_folder_entry.id, "location": new_location},
        )
