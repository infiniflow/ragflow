...
async def update_doc(tenant_id, dataset_id, document_id):
    ...
    if "chunk_method" in req:
        ...
        if doc.token_num > 0:
            ...
            if not e:
                return get_error_data_result(message="Document not found!")
            providers.doc_store.conn.delete({"doc_id": doc.id}, search.index_name(tenant_id), dataset_id)
    ...
    if "enabled" in req:
        ...
        if doc.status != req["enabled"]:
            try:
                ...
                providers.doc_store.conn.update({"doc_id": doc.id}, {"available_int": status}, search.index_name(kb.tenant_id), doc.kb_id)
...
async def download(tenant_id, dataset_id, document_id):
    ...
    file_stream = providers.storage.conn.get(doc_id, doc_location)
    if not file_stream:
...
async def parse(tenant_id, dataset_id):
    ...
    for id in doc_list:
        ...
        providers.doc_store.conn.delete({"doc_id": id}, search.index_name(tenant_id), dataset_id)
...
async def stop_parsing(tenant_id, dataset_id):
    ...
    for id in doc_list:
        ...
        DocumentService.update_by_id(id, info)
        providers.doc_store.conn.delete({"doc_id": doc[0].id}, search.index_name(tenant_id), dataset_id)
        success_count += 1
    if duplicate_messages:
        if success_count > 0:
            return get_result(
...
def list_chunks(tenant_id, dataset_id, document_id):
    ...
    if req.get("id"):
        chunk = providers.doc_store.conn.get(req.get("id"), search.index_name(tenant_id), [dataset_id])
        if not chunk:
    ...
    elif providers.doc_store.conn.index_exist(search.index_name(tenant_id), dataset_id):
        sres = providers.retriever.conn.search(query, search.index_name(tenant_id), [dataset_id], emb_mdl=None, highlight=True)
...
async def add_chunk(tenant_id, dataset_id, document_id):
    ...
    providers.doc_store.conn.insert([d], search.index_name(tenant_id), dataset_id)
...
async def rm_chunk(tenant_id, dataset_id, document_id):
    ...
    if "chunk_ids" in req:
        ...
        unique_chunk_ids = []
        duplicate_messages = []
    chunk_number = providers.doc_store.conn.delete(condition, search.index_name(tenant_id), dataset_id)
    if chunk_number != 0:
...
async def update_chunk(tenant_id, dataset_id, document_id, chunk_id):
    ...
    responses:
      200:
        ...
        schema:
          type: object
    """
    chunk = providers.doc_store.conn.get(chunk_id, search.index_name(tenant_id), [dataset_id])
    if chunk is None:
    ...
    d["q_%d_vec" % len(v)] = v.tolist()
    providers.doc_store.conn.update({"id": chunk_id}, d, search.index_name(tenant_id), dataset_id)
    return get_result()
...
async def retrieval_test(tenant_id):
    ...
    try:
        ...
        ranks = providers.retriever.conn.retrieval(
            question,
            embd_mdl,
            tenant_ids,
            kb_ids,
        ...
        if toc_enhance:
            chat_mdl = LLMBundle(kb.tenant_id, LLMType.CHAT)
            cks = await providers.retriever.conn.retrieval_by_toc(question, ranks["chunks"], tenant_ids, chat_mdl, size)
            if cks:
                ranks["chunks"] = cks
        if use_kg:
            ck = await providers.kg_retriever.conn.retrieval(question, [k.tenant_id for k in kbs], kb_ids, embd_mdl, LLMBundle(kb.tenant_id, LLMType.CHAT))
