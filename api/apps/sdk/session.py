...
def chat_completion_openai_like(tenant_id, chat_id):
    ...
    This function allows users to interact with a model to get responses based on a series of messages. 
    If `stream` is set to True, the response will be streamed in chunks, mimicking the OpenAI-style API.
    ...
    if len(messages) < 1:
        return get_error_data_result(f"You have to provide messages")
    ...
    msg = None
    msg = [m for m in messages if m["role"] != "system" and (m["role"] != "assistant" or not msg)]
