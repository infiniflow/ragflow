...
def create_an_agent_session(tenant_id,agent_id,user_id):
    e, cvs = UserCanvasService.get_by_id(agent_id)
    if not e:
        return "Agent not found."
    if cvs.user_id != tenant_id:
        return "You do not own the agent."
    language=request.json.get("language")
    file_value=request.json.get("file_value")
    if not isinstance(cvs.dsl, str):
        cvs.dsl = json.dumps(cvs.dsl, ensure_ascii=False)
    canvas = Canvas(cvs.dsl, tenant_id)
    prologue=canvas.get_prologue()
    if canvas.judge_translation():
        if not language or not file_value:
            return "`language` and `file_value` are required "
...
def agent_completion(tenant_id, agent_id):
    ...
    if not req.get("session_id"):
        ...
        if not conv :
            return get_error_data_result(message=f"You don't own the session {req['session_id']}")
        conv = conv[0]
        canvas = Canvas(json.dumps(conv.dsl), tenant_id)
    session_id =conv.id
...
def list_agent_session(agent_id,tenant_id):
    ...
    id = request.args.get("id")
    if not API4ConversationService.query(id=id,dialog_id=agent_id):
