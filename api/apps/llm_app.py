...
def add_llm():
    ...
    if llm["model_type"] == LLMType.EMBEDDING.value:
        mdl = EmbeddingModel[factory](
            key=llm['api_key'] if factory in ["VolcEngine", "Bedrock","OpenAI-API"] else None,
        ...
        mdl = CvModel[factory](
            key=llm["api_key"] if factory in ["OpenAI-API"] else None, model_name=llm["llm_name"], base_url=llm["api_base"]
        )
        try:
            img_url = (
