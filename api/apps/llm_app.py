...
async def get_factory_models(factory: str):
    ...
    try:
        ...
        if is_dynamic_provider(factory):
            ...
            logging.debug(f"[CATEGORY FILTER] Factory: {factory}, Category parameter: {category_filter}, Query args: {request.args}")
            ...
            logging.debug(f"[CATEGORY FILTER] Available categories: {list(models_by_category.keys())}, Count per category: {{{', '.join([f'{k}: {len(v)}' for k, v in models_by_category.items()])}}}")
            ...
            if category_filter and category_filter in models_by_category:
                ...
                logging.debug(f"[CATEGORY FILTER] Filtered to {len(filtered_models)} models for category '{category_filter}'")
            else:
