...
def get_login_channels():
  ...
  try:
    ...
    logging.exception(e)
    return get_json_result(
        data=[], message=f"Load channels failure, error: {str(e)}", code=settings.RetCode.EXCEPTION_ERROR)
...
def forget_send_otp():
  ...
  if last_ts:
    ...
    if remaining > 0:
      return get_json_result(
          data=False, code=settings.RetCode.NOT_EFFECTIVE, message=f"you still have to wait {remaining} seconds")
...
def forget():
  ...
  if not all([email, otp, new_pwd, new_pwd2]):
    return get_json_result(
        data=False, code=settings.RetCode.ARGUMENT_ERROR, message="email, otp and passwords are required")
