...
class EvaDocumentChangeService:
        ...
        *,
        ...
        if requested_draft is not None and not allow_prefilled_draft:
            raise ValidationError(
                ...
                "EVA drafts are generated by the agent and cannot be supplied manually",
            )
    ...
    def generate_draft(
        ...
        *,
        ...
        if not isinstance(raw, dict):
            raise ValidationError("INVALID_EVA_GENERATION", "Request body must be an object")
        ...
        if len(change_request) > 50_000:
            error = ValidationError("INVALID_EVA_GENERATION", "Combined change request is too long", {"maximum": 50_000})
        ...
        try:
            ...
            if draft_hash == change.base_content_hash:
                raise ValidationError(
                    ...
                    "The agent did not produce any document changes; clarify the task and try again",
                )
            draft_html = _markdown_to_html(draft_markdown)
        except Exception as error:
            mapped = error if isinstance(error, BusinessDocumentError) else BusinessDocumentError("EVA_AI_GENERATION_FAILED", "The agent could not prepare the EVA draft", 502)
            cls._restore_after_external_failure(
                change.id,
                reservation_version,
                EvaChangeState.GENERATING_DRAFT,
        ...
        with BusinessDocumentEvaChange._meta.database.atomic():
            ...
            if change.workflow_state == EvaChangeState.GENERATING_DRAFT.value:
                ...
                if age_ms < _EXTERNAL_OPERATION_TIMEOUT_MS:
                    raise ConflictError(
                        "EVA_CHANGE_BUSY",
                        "The agent is still preparing the EVA draft",
                        {"state": change.workflow_state, "retry_after_ms": _EXTERNAL_OPERATION_TIMEOUT_MS - max(age_ms, 0)},
                    )
                ...
                raise ConflictError(
                    ...
                    "The EVA change request is not ready for agent generation",
