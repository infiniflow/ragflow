...
async def save_to_memory(memory_id: str, message_dict: dict):
    ...
    extracted_content = (
        await extract_by_llm(
            tenant_id, memory.llm_id, {"temperature": memory.temperature}, get_memory_type_human(memory.memory_type), message_dict.get("user_input", ""), message_dict.get("agent_response", "")
        )
...
async def extract_by_llm(
    tenant_id: str, llm_id: str, extract_conf: dict, memory_type: List[str], user_input: str, agent_response: str, system_prompt: str = "", user_prompt: str = "", task_id: str = None
) -> List[dict]:
