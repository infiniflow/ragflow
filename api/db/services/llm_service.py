...
class LLMBundle(LLM4Tenant):
    ...
    def _should_use_tools(self, method_name: str) -> bool:
        return bool(self.is_tools and getattr(self.mdl, "is_tools", False) and getattr(self.mdl, "tools", None) and callable(getattr(self.mdl, method_name, None)))

    async def async_chat(self, system: str, history: list, gen_conf: dict = {}, **kwargs):
        if self._should_use_tools("async_chat_with_tools"):
    ...
    async def async_chat_streamly(self, system: str, history: list, gen_conf: dict = {}, **kwargs):
        total_tokens = 0
        ans = ""
        if self._should_use_tools("async_chat_streamly_with_tools"):
