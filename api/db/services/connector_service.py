...
class SyncLogsService(CommonService):
    ...
    def duplicate_and_parse(cls, kb, docs, tenant_id, src, auto_parse=True):
        ...
        errs = []
        files = [FileObj(id=hash128(d["id"]), filename=d["semantic_identifier"]+(f"{d['extension']}" if d["semantic_identifier"][::-1].find(d['extension'][::-1])<0 else ""), blob=d["blob"]) for d in docs]
        doc_ids = []
