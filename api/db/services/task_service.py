    ...
    def get_task(cls, task_id, doc_ids=[]):
        ...
        if doc["retry_count"] >= 3:
            if not doc.get("task_type"):
                remaining = credit_doc_chunking_task(doc["doc_id"], doc["id"])
                if remaining == 0:
                    logging.warning(
                        "Chunking counter reached zero after abandoned task %s for doc %s; "
                        "post-chunking finalizer was not started.",
                        doc["id"],
                        doc["doc_id"],
                    )
                clear_doc_chunking_counter(doc["doc_id"])
            return None
