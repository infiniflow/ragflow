...
class BaseModel(Model):
    ...
    def to_human_model_dict(self, only_primary_with: Optional[list[str]] = None):
        ...
        human_model_dict = {}
        for k in self.get_primary_keys_name():
            if k in model_dict:
                ...
                if is_continuous_field(type(getattr(cls, attr_name))):
                    if len(f_v) == 2:
                        ...
                        if lower_bound is not None and upper_bound is not None:
                            filters.append(cls.getter_by(attr_name).between(lower_bound, upper_bound))
                        elif lower_bound is not None:
                            filters.append(operator.attrgetter(attr_name)(cls) >= lower_bound)
                        elif upper_bound is not None:
                            filters.append(operator.attrgetter(attr_name)(cls) <= upper_bound)
                else:
    ...
    def insert(cls, __data=None, **insert):
        ...
        if insert:
            timestamp = current_timestamp()
            insert["create_time"] = timestamp
            if "create_date" in cls._meta.combined:
                insert["create_date"] = timestamp_to_date(timestamp)
