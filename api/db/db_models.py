...
class Dialog(DataBaseModel):
    ...
    status = CharField(max_length=1, null=True, help_text="is it validate(0: wasted, 1: validate)", default="1", index=True)
    prompt_variant_id = CharField(max_length=32, null=True, help_text="FK to prompt_variant; tracks which variant is active")
...
class Conversation(DataBaseModel):
    ...
    compressed_message = JSONField(null=True, help_text="rolling compression summary payload")
    compression_cursor = IntegerField(default=0, help_text="index of oldest uncompressed message")
    token_tally = IntegerField(default=0, help_text="cumulative prompt+completion tokens for this session")
...
class PromptVariant(DataBaseModel):
    """A candidate or active prompt text generated by the optimisation pipeline."""
    id = CharField(max_length=32, primary_key=True)
    source_type = CharField(max_length=16, null=False, help_text="dialog|component", index=True)
    source_id = CharField(max_length=32, null=False, index=True, help_text="dialog_id or canvas component id")
    prompt_text = TextField(null=False, help_text="full prompt text of this variant")
    score = FloatField(null=True, help_text="benchmark score; NULL until evaluated")
    status = CharField(max_length=16, null=False, default="candidate", index=True,
                       help_text="candidate|active|retired")
    promoted_at = BigIntegerField(null=True, help_text="timestamp when this variant was promoted to active")

    class Meta:
        db_table = "prompt_variant"


class PromptOptimisationRun(DataBaseModel):
    """Tracks a single execution of the prompt optimisation pipeline."""
    id = CharField(max_length=32, primary_key=True)
    tenant_id = CharField(max_length=32, null=False, index=True)
    source_type = CharField(max_length=16, null=False, help_text="dialog|component", index=True)
    source_id = CharField(max_length=32, null=False, index=True)
    eval_dataset_id = CharField(max_length=32, null=True, help_text="evaluation dataset used for benchmarking")
    triggered_by = CharField(max_length=16, null=False, default="manual",
                             help_text="manual|scheduled|threshold")
    ...
    winner_variant_id = CharField(max_length=32, null=True,
                                  help_text="FK to prompt_variant; set on completion")

    class Meta:
        db_table = "prompt_optimisation_run"
