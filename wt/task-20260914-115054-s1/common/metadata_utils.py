...
def _constrain_doc_ids(base_doc_ids: list[str] | None, filtered_doc_ids: list[str]) -> list[str]:
    if not base_doc_ids:
        return dedupe_list(filtered_doc_ids)
...
async def apply_meta_data_filter(
    ...
    ``base_doc_ids`` is the caller's document scope. Filter hits narrow it by
    intersection instead of widening it: only documents that are both inside
    the scope and matched by the filter are searched, and a filter matching
    nothing (or nothing inside the scope) yields the ["-999"] no-match
    sentinel so retrieval returns no documents instead of falling back to the
    unfiltered scope.

    Returns:
        list of doc_ids, ["-999"] when a filter matches no document, or None
        when auto/semi_auto generate no usable filter and no base scope was
        supplied.
    """
    ...
    def _narrow_scope(conditions: list[dict], logic: str) -> list[str]:
        if not conditions:
            return doc_ids
        constrained = _constrain_doc_ids(base_doc_ids, _run_metadata_filter(conditions, logic))
        return constrained or ["-999"]

    if method == "auto":
        filters: dict = await gen_meta_filter(chat_mdl, _get_metas(), question)
        logging.debug(f"Metadata filter(auto) generated: {filters}")
        doc_ids = _narrow_scope(filters.get("conditions") or [], filters.get("logic", "and"))
        if not doc_ids:
            return None
        ...
        doc_ids = _narrow_scope(filters, meta_data_filter.get("logic", "and"))
