...
def _constrain_doc_ids(base_doc_ids: list[str], filtered_doc_ids: list[str]) -> list[str]:
    matched = list(dict.fromkeys(filtered_doc_ids))
    if not base_doc_ids:
        return matched
    if not matched:
        return []
    matched_set = set(matched)
    return [doc_id for doc_id in dict.fromkeys(base_doc_ids) if doc_id in matched_set]
...
async def apply_meta_data_filter(
    ...
    ``base_doc_ids`` is the caller's document scope: the filter narrows it to
    the matching documents instead of widening it with every filter hit. An
    empty base scope leaves the filter hits as the scope.

    Returns:
        the narrowed doc_ids, ["-999"] when manual filters match nothing, or
        None when auto/semi_auto filters match nothing.
    """
    ...
    if method == "auto":
        ...
        doc_ids = _constrain_doc_ids(doc_ids, _run_metadata_filter(filters["conditions"], filters.get("logic", "and")))
        if not doc_ids:
            return None
        ...
        doc_ids = _constrain_doc_ids(doc_ids, _run_metadata_filter(filters, meta_data_filter.get("logic", "and")))
