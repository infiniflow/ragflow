...
// compilerDSL returns a minimal DSL resembling the "compiler" template's
// Compiler component.
...
func TestCleanComponentParams_KeepsCompilerLLMRuntimeParams(t *testing.T) {
	dslJSON := compilerDSL(t)
	raw := map[string]any{
		"Compiler:NewBoxesLove": map[string]any{
			"compilation_template_group_id": "268d2648a80611f1",
			"llm_id":                        "9c94127c8fb311f1",
			"parameter":                     "Custom",
			"temperature":                   0.7,
			...
			"thinking":                      "enabled",
			"keywords":                      map[string]any{"top_n": 5},
			"no_such_param":                 1,
		},
	}
	result := CleanComponentParams(dslJSON, raw)
	...
	if !ok {
		t.Fatalf("expected Compiler:NewBoxesLove in result, got %v", result)
	}
	for _, key := range []string{
		"compilation_template_group_id", "llm_id",
		"parameter", "temperature", "temperatureEnabled",
		"top_p", "topPEnabled",
		...
		if _, ok := params[key]; !ok {
			t.Errorf("expected LLM runtime key %q to survive CleanComponentParams, got %#v", key, params)
		}
	}
	if _, ok := params["keywords"]; ok {
		t.Error("expected extractor feature group 'keywords' to be dropped for Compiler")
	}
	if _, ok := params["no_such_param"]; ok {
		t.Error("expected unknown param key to be dropped for Compiler")
	}
}

func TestBuildParserConfig_CompilerRuntimeParamsRoundTrip(t *testing.T) {
	dslJSON := compilerDSL(t)
	raw := map[string]any{
		"Compiler:NewBoxesLove": map[string]any{
	...
	if params["compilation_template_group_id"] != "group-1" {
		t.Errorf("expected override compilation_template_group_id=group-1, got %v", params["compilation_template_group_id"])
	}
	if params["temperature"] != 0.7 || params["temperatureEnabled"] != true {
		t.Errorf("expected temperature overrides to survive, got %#v", params)
	}
	if params["thinking"] != "enabled" {
		t.Errorf("expected thinking=enabled to survive, got %v", params["thinking"])
	}
	if v, ok := params["plan"]; !ok || v != false {
		t.Errorf("expected DSL default plan=false to be preserved, got %#v", params["plan"])
	}
}
