...
CONSUMER_NO_END=0
WORKERS=1
GUNICORN_MODE=${GUNICORN_MODE:-0} # Start RAGFlow in Gunicorn mode, default to 0 for Flask default WSGI server
...
if [[ "${ENABLE_WEBSERVER}" -eq 1 ]]; then
  ...
  if [[ "${GUNICORN_MODE}" -eq 1 ]]; then
      ...
      exec gunicorn --config /ragflow/conf/gunicorn.conf.py 'api.wsgi:application'
        ...
        echo "Starting ragflow_server with Flask (Development Mode)..."
        while true; do
