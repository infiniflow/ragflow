...
show_help() {
    ...
    echo ""
    echo "OPERATIONS:"
    echo "  backup   - Create backup of the RAGFlow data volumes listed below"
    ...
    echo ""
    echo "  Those four are the whole set. DOC_ENGINE chooses where your indices live,"
    echo "  and only 'elasticsearch' keeps them in ${DEFAULT_PROJECT_NAME}_esdata01. Under 'infinity',"
    echo "  'opensearch' or 'serenedb' the index volume is not part of this backup, and"
    echo "  because no es_backup.tar.gz is written, a later 'restore' stops on the"
    echo "  missing file. Back up the index volume separately until that is handled."
    echo ""
    echo "NOTE:"
