...
- `DEEPDOC_URL`  
  URL for the deepdoc vision API serving DLA (layout analysis), OCR (text detection/recognition), and TSR (table structure recognition). The `deepdoc` service in `docker-compose.yml` provides this endpoint. Defaults to `http://deepdoc:8124`. When unset, the parser falls back to inline ONNX Runtime inference.
...
- `deepdoc`  
  ...
  - `port`: Serving port inside the container. Defaults to `8124`.
  - Health check: `curl -f http://localhost:8124/health` every 10s.
