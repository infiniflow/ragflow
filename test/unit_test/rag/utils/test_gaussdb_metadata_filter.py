...
def test_tc_mgf_005_nested_key_path_uses_jsonb_path_literal():
    sql = _assert_filter(
        ...
        "and",
        "(lower(meta_fields #>> '{profile,name}') = %s OR "
        "jsonb_exists(meta_fields #> '{profile,name}', %s))",
        ["alice", "alice"],
    )
...
def test_tc_mgf_006_custom_jsonb_column_is_used_for_translation():
    translated = GaussDBMetaFilterTranslator(jsonb_column="chunk_data").translate(
...
def test_tc_mgf_104_numeric_comparisons_use_canonical_sql_operators():
    ...
    for filter_op, sql_op in cases:
        sql = _assert_filter(
            ...
            "and",
            "(meta_fields #>> '{amount}' ~ '^-?[0-9]+(\\.[0-9]+)?$' AND "
            f"(meta_fields #>> '{{amount}}')::DOUBLE PRECISION {sql_op} %s)",
            [50],
        )
...
def test_tc_mgf_107_not_contains_requires_existing_key_and_negated_contains():
    sql = _assert_filter(
        ...
        "and",
        "((meta_fields #> '{cust}') IS NOT NULL AND "
        "(jsonb_exists(meta_fields #> '{cust}', %s) OR "
        "lower(meta_fields #>> '{cust}') LIKE %s ESCAPE '\\') IS NOT TRUE)",
        ["acme", "%acme%"],
    )
...
def test_tc_mgf_401_and_logic_joins_three_mixed_operators_in_order():
    sql = _assert_filter(
        ...
        "(lower(meta_fields #>> '{status}') = %s OR jsonb_exists(meta_fields #> '{status}', %s)) AND "
        "(meta_fields #>> '{amount}' ~ '^-?[0-9]+(\\.[0-9]+)?$' AND "
        "(meta_fields #>> '{amount}')::DOUBLE PRECISION > %s) AND "
        "(jsonb_exists(meta_fields #> '{cust}', %s) OR "
        "lower(meta_fields #>> '{cust}') LIKE %s ESCAPE '\\')",
...
def test_tc_mgf_404_nested_key_exists_and_jsonb_number_predicate_join_with_and():
    sql = _assert_filter(
        ...
        "and",
        "((meta_fields #> '{a,b}') IS NOT NULL AND "
        "(lower(meta_fields #>> '{a,b}') = %s OR jsonb_exists(meta_fields #> '{a,b}', %s)) IS NOT TRUE) AND "
        "(meta_fields #> '{a,c}' @> %s::jsonb)",
        ["x", "x", "1"],
    )
...
def test_tc_mgf_405_special_key_segments_are_encoded_without_sql_injection():
    ...
    sql = _assert_filter(
        ...
        "and",
        f"((meta_fields #> {path}) IS NOT NULL AND "
        f"(lower(meta_fields #>> {path}) = %s OR jsonb_exists(meta_fields #> {path}, %s)) IS NOT TRUE)",
        ["x", "x"],
    )
...
def test_tc_mgf_504_operator_normalization_tolerates_case_and_whitespace():
    cases = [
        ...
        (
            "Contains",
            "(jsonb_exists(meta_fields #> '{status}', %s) OR "
...
def test_tc_mgf_704_all_supported_operators_normalize_and_translate():
    ...
    cases = [
        ...
        (
            ...
            "(meta_fields #> '{status}') IS NOT NULL AND "
            "(jsonb_exists(meta_fields #> '{status}', %s) OR "
            "lower(meta_fields #>> '{status}') LIKE %s ESCAPE '\\') IS NOT TRUE",
            ["x", "%x%"],
        ),
        (
            "start with",
