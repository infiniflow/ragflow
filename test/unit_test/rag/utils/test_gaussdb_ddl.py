...
def test_tc_wrt_904_vector_column_ddls_match_exact_snapshot():
    ...
    assert statements == [
        'ALTER TABLE "test_schema"."ragflow_unit" ADD COLUMN IF NOT EXISTS q_1024_vec floatvector(1024) '
        "DEFAULT (array_fill(0, ARRAY[1024])::text::floatvector(1024))",
...
def test_tc_wrt_004_validate_vector_dim_accepts_gaussdb_boundaries(dim, diskann_options):
    ...
    assert builder.build_vector_column_ddls("ragflow_t1", dim) == [
        f'ALTER TABLE "test_schema"."ragflow_t1" ADD COLUMN IF NOT EXISTS q_{dim}_vec floatvector({dim}) '
        f"DEFAULT (array_fill(0, ARRAY[{dim}])::text::floatvector({dim}))",
        f'ALTER TABLE "test_schema"."ragflow_t1" ADD COLUMN IF NOT EXISTS '
        f"q_{dim}_vec_valid BOOLEAN DEFAULT FALSE NOT NULL",
    ]
    assert builder.build_diskann_index_ddl("ragflow_t1", dim) == (
        f'CREATE INDEX IF NOT EXISTS "idx_gdb_ragflow_t1_q_{dim}_vec_diskann" '
        f'ON "test_schema"."ragflow_t1" USING gsdiskann (q_{dim}_vec COSINE) WITH ({diskann_options})'
    )
...
def test_tc_wrt_009_diskann_dimension_matrix_matches_exact_ddl(dim, expected_options):
    ...
    assert ddl == (
        f'CREATE INDEX IF NOT EXISTS "idx_gdb_ragflow_tenant_a_q_{dim}_vec_diskann" '
        f'ON "public"."ragflow_tenant_a" USING gsdiskann (q_{dim}_vec COSINE) WITH ({expected_options})'
    )
...
def test_tc_wrt_911_long_table_identifier_produces_bounded_distinct_index_names():
    ...
    for column, index_name, statement in zip(index_columns, expected_names[:5], regular_ddls, strict=True):
        assert statement == (
            ...
            f'ON "test_schema"."{table}" ({column})'
        )
    assert diskann_ddl == (
        f'CREATE INDEX IF NOT EXISTS "{expected_names[-1]}" '
        f'ON "test_schema"."{table}" USING gsdiskann (q_3072_vec COSINE) '
        "WITH (subgraph_count=1, enable_vector_copy=false)"
    )
...
def test_tc_wrt_912_doc_meta_table_ddl_uses_id_pk_ustore_and_kb_index():
    ...
    assert [_normalize_ddl(statement) for statement in statements] == [
        ...
        'CREATE INDEX IF NOT EXISTS "idx_gdb_ragflow_doc_meta_tenant_a_kb_id" '
        'ON "public"."ragflow_doc_meta_tenant_a" (kb_id)',
    ]
...
def test_tc_wrt_913_regular_index_ddls_match_exact_column_matrix():
    ...
    assert statements == [
        f'CREATE INDEX IF NOT EXISTS "idx_gdb_ragflow_tenant_a_{column}" '
        f'ON "public"."ragflow_tenant_a" ({column})'
