...
def _validator(kb_ids=None, field_map=None, fetch_size=128):
    table = ExposedGaussDBTable.from_field_map(
        ...
        field_map={"amount": "number", "dept": "string", "customer,name": "string"}
...
def test_tc_sql_101_validator_allows_single_table_select_and_returns_columns():
    validated = _validator(field_map={}).validate_and_patch(
        "SELECT doc_id, docnm_kwd FROM ragflow_t1 WHERE kb_id = 'kb-1' ORDER BY doc_id LIMIT 5"
    )
...
def test_tc_sql_102_validator_allows_readonly_cte_and_exposed_output_aliases():
    baseline = _validator(field_map={}).validate_and_patch(
        "WITH x AS (SELECT doc_id FROM ragflow_t1 WHERE kb_id = 'kb-1') SELECT doc_id FROM x"
    )
    alias = _validator(field_map={"amount": "number"}).validate_and_patch(
        "WITH rows AS (SELECT chunk_data #>> '{amount}' AS amount, kb_id "
        "FROM ragflow_t1 WHERE kb_id = 'kb-1') SELECT amount FROM rows"
    )
    qualified_alias = _validator(field_map={"amount": "number"}).validate_and_patch(
        "WITH rows AS (SELECT chunk_data #>> '{amount}' AS amount, kb_id "
        "FROM ragflow_t1 WHERE kb_id = 'kb-1') SELECT rows.amount FROM rows"
    )

    assert baseline.sql == (
        "WITH x AS (SELECT doc_id FROM ragflow_t1 WHERE kb_id = 'kb-1') "
        "SELECT doc_id FROM x LIMIT 128"
    )
    assert baseline.columns == ["doc_id"]
    assert baseline.is_aggregation is False
    assert alias.sql == (
        "WITH rows AS (SELECT chunk_data #>> '{amount}' AS amount, kb_id FROM ragflow_t1 "
        "WHERE kb_id = 'kb-1') SELECT amount FROM rows LIMIT 128"
    )
    assert alias.columns == ["amount"]
    assert alias.is_aggregation is False
    assert qualified_alias.sql == (
        "WITH rows AS (SELECT chunk_data #>> '{amount}' AS amount, kb_id FROM ragflow_t1 "
        "WHERE kb_id = 'kb-1') SELECT rows.amount FROM rows LIMIT 128"
    )
...
def test_tc_sql_104_validator_rejects_dml_including_nested_cte():
    scenarios = {
        ...
        "WITH x AS (DELETE FROM ragflow_t1 RETURNING doc_id) SELECT doc_id FROM x": (
            "SQL contains a non-read-only expression"
        ),
    }
...
def test_tc_sql_115_validator_rejects_oceanbase_json_functions():
    scenarios = {
        "SELECT json_extract_string(chunk_data, '$.amount') FROM ragflow_t1": (
            "function json_extract_string is not allowed"
        ),
        "SELECT json_extract(chunk_data, '$.amount') FROM ragflow_t1": (
            "only GaussDB #> / #>> JSONB operators are allowed"
        ),
        "SELECT json_extract_isnull(chunk_data, '$.amount') FROM ragflow_t1": (
            "function json_extract_isnull is not allowed"
        ),
    }
    for sql, message in scenarios.items():
        _assert_unsafe_sql(sql, message)
...
def test_tc_sql_116_validator_rejects_forbidden_jsonb_set_returning_functions():
    scenarios = {
        ...
        "SELECT jsonb_array_elements(chunk_data) FROM ragflow_t1": (
            "function jsonb_array_elements is not allowed"
        ),
        "SELECT jsonb_to_recordset(chunk_data) FROM ragflow_t1": (
            "function jsonb_to_recordset is not allowed"
        ),
    }
    for sql, message in scenarios.items():
        _assert_unsafe_sql(sql, message, _validator(field_map={}))
...
def test_tc_sql_117_validator_rejects_join_even_when_both_aliases_are_scoped():
    _assert_unsafe_sql(
        ...
        "WHERE a.kb_id = 'kb-1' AND b.kb_id = 'kb-1'",
...
def test_tc_sql_118_validator_rejects_union_as_non_select_root():
    _assert_unsafe_sql(
        "SELECT doc_id FROM ragflow_t1 WHERE kb_id = 'kb-1' "
        "UNION SELECT doc_id FROM ragflow_t1 WHERE kb_id = 'kb-2'",
        "only SELECT statements are allowed",
        _validator(field_map={}),
    )
...
def test_tc_sql_120_validator_rejects_dynamic_or_wrong_source_jsonb_paths():
    scenarios = {
        "SELECT doc_id FROM ragflow_t1 WHERE chunk_data #>> ('{' || param || '}') = '100'": (
            "dynamic JSONB path is not allowed"
        ),
        "SELECT doc_id, chunk_data #>> path_col FROM ragflow_t1 WHERE kb_id = 'kb-1'": (
            "dynamic JSONB path is not allowed"
        ),
        "SELECT doc_id, other_data #>> '{amount}' FROM ragflow_t1 WHERE kb_id = 'kb-1'": (
            "only chunk_data JSONB paths are allowed"
        ),
    }
    for sql, message in scenarios.items():
        _assert_unsafe_sql(sql, message)
...
def test_tc_sql_202_validator_injects_multi_kb_boundary():
    validated = _validator(kb_ids=["kb-1", "kb-2"], field_map={}).validate_and_patch(
        "SELECT doc_id FROM ragflow_t1"
    )

    assert validated.sql == (
        "SELECT doc_id FROM ragflow_t1 WHERE kb_id IN ('kb-1', 'kb-2') LIMIT 128"
    )
    assert validated.columns == ["doc_id"]
    assert validated.is_aggregation is False
...
def test_tc_sql_203_validator_preserves_allowed_kb_boundary_shapes():
    equality = _validator(field_map={}).validate_and_patch(
        "SELECT doc_id FROM ragflow_t1 WHERE kb_id = 'kb-1'"
    )
    reversed_equality = _validator(field_map={}).validate_and_patch(
        "SELECT doc_id FROM ragflow_t1 WHERE 'kb-1' = kb_id"
    )
    in_boundary = _validator(kb_ids=["kb-1", "kb-2"], field_map={}).validate_and_patch(
        "SELECT doc_id FROM ragflow_t1 WHERE kb_id IN ('kb-1', 'kb-2')"
    )
    predicate_before_boundary = _validator(
        kb_ids=["kb-1", "kb-2"], field_map={}
    ).validate_and_patch(
        "SELECT doc_id FROM ragflow_t1 WHERE doc_id IN ('doc1', 'doc2') AND kb_id IN ('kb-1')"
    )
    ...
    assert in_boundary.sql == (
        "SELECT doc_id FROM ragflow_t1 WHERE kb_id IN ('kb-1', 'kb-2') LIMIT 128"
    )
    assert predicate_before_boundary.sql == (
        "SELECT doc_id FROM ragflow_t1 WHERE doc_id IN ('doc1', 'doc2') "
        "AND kb_id IN ('kb-1') LIMIT 128"
    )
...
def test_tc_sql_205_validator_rejects_non_positive_or_dynamic_kb_predicates():
    scenarios = {
        "SELECT doc_id FROM ragflow_t1 WHERE NOT kb_id = 'kb-1'": (
            "kb_id boundary must be a positive top-level predicate"
        ),
        "SELECT doc_id FROM ragflow_t1 WHERE (kb_id = 'kb-1') IS FALSE": (
            "kb_id boundary must be a positive top-level predicate"
        ),
        "SELECT doc_id FROM ragflow_t1 WHERE kb_id IN (doc_id)": (
            "kb_id IN must use static string literals"
        ),
...
def test_tc_sql_209_validator_requires_each_cte_base_scope_to_prove_kb_boundary():
    ...
    _assert_unsafe_sql(
        "WITH rows AS (SELECT doc_id, 'kb-1' AS kb_id FROM ragflow_t1) "
        "SELECT doc_id FROM rows WHERE kb_id = 'kb-1'",
        "each base table scope must include a kb_id boundary",
        validator,
    )

    validated = validator.validate_and_patch(
        "WITH x AS (SELECT doc_id FROM ragflow_t1 WHERE kb_id = 'kb-1') SELECT doc_id FROM x"
    )
    assert validated.sql == (
        "WITH x AS (SELECT doc_id FROM ragflow_t1 WHERE kb_id = 'kb-1') "
...
def test_tc_sql_209_validator_requires_each_subquery_scope_to_prove_kb_boundary(correlation_predicate):
    validator = _validator(field_map={})
    outer_missing = (
        "SELECT outer_t.doc_id FROM ragflow_t1 AS outer_t WHERE EXISTS ("
        "SELECT 1 FROM ragflow_t1 AS inner_t WHERE "
        f"{correlation_predicate}inner_t.kb_id = 'kb-1')"
    )
    inner_missing = (
        "SELECT outer_t.doc_id FROM ragflow_t1 AS outer_t WHERE outer_t.kb_id = 'kb-1' AND EXISTS ("
        "SELECT 1 FROM ragflow_t1 AS inner_t WHERE "
        f"{correlation_predicate}inner_t.doc_id = 'doc-1')"
    )
    legal = (
        "SELECT outer_t.doc_id FROM ragflow_t1 AS outer_t WHERE outer_t.kb_id = 'kb-1' AND EXISTS ("
        "SELECT 1 FROM ragflow_t1 AS inner_t WHERE "
    ...
    cross_kb = (
        "SELECT outer_t.doc_id FROM ragflow_t1 AS outer_t WHERE outer_t.kb_id = 'kb-1' AND EXISTS ("
        "SELECT 1 FROM ragflow_t1 AS inner_t WHERE "
        f"{correlation_predicate}inner_t.kb_id = 'kb-other')"
    )
...
def test_tc_sql_302_validator_caps_large_limit_to_default_limit():
    validated = _validator(field_map={}).validate_and_patch(
        "SELECT doc_id FROM ragflow_t1 WHERE kb_id = 'kb-1' LIMIT 200"
    )
...
def test_tc_sql_303_validator_preserves_limit_under_default_limit():
    validated = _validator(field_map={}).validate_and_patch(
        "SELECT doc_id FROM ragflow_t1 WHERE kb_id = 'kb-1' LIMIT 100"
    )
...
def test_tc_sql_305_validator_rejects_complex_scope_before_limit_patch():
    ...
    scenarios = (
        ...
        "SELECT doc_id FROM ragflow_t1 WHERE kb_id = 'kb-1' "
        "OR chunk_data #>> '{status}' = 'active' LIMIT 500",
    )
    for sql in scenarios:
        _assert_unsafe_sql(
            sql,
...
def test_tc_sql_307_validator_caps_fetch_first_to_default_limit():
    validated = _validator(field_map={}).validate_and_patch(
        "SELECT doc_id FROM ragflow_t1 WHERE kb_id = 'kb-1' FETCH FIRST 500 ROWS ONLY"
    )

    assert validated.sql == (
        "SELECT doc_id FROM ragflow_t1 WHERE kb_id = 'kb-1' FETCH FIRST 128 ROWS ONLY"
    )
    assert validated.columns == ["doc_id"]
    assert validated.is_aggregation is False
...
def test_tc_sql_308_clause_tokens_in_literals_or_subqueries_do_not_block_ast_patches():
    limit_field = _validator(field_map={"limit": "string"}).validate_and_patch(
        "SELECT doc_id FROM ragflow_t1 WHERE kb_id = 'kb-1' AND chunk_data #>> '{limit}' = '100'"
    )
    subquery_limit = _validator(field_map={}).validate_and_patch(
        "SELECT doc_id FROM ragflow_t1 WHERE kb_id = 'kb-1' AND EXISTS (SELECT 1 LIMIT 1)"
    )
    forbidden_word_literal = _validator(field_map={"dept": "string"}).validate_and_patch(
        "SELECT doc_id FROM ragflow_t1 WHERE kb_id = 'kb-1' AND chunk_data #>> '{dept}' = 'update'"
    )
    order_word_literal = _validator(field_map={"dept": "string"}).validate_and_patch(
        "SELECT doc_id FROM ragflow_t1 WHERE chunk_data #>> '{dept}' = 'order by dept' ORDER BY doc_id"
    )

    assert limit_field.sql == (
        "SELECT doc_id FROM ragflow_t1 WHERE kb_id = 'kb-1' "
        "AND chunk_data #>> '{limit}' = '100' LIMIT 128"
    )
    assert subquery_limit.sql == (
        "SELECT doc_id FROM ragflow_t1 WHERE kb_id = 'kb-1' "
        "AND EXISTS(SELECT 1 LIMIT 1) LIMIT 128"
    )
    assert forbidden_word_literal.sql == (
        "SELECT doc_id FROM ragflow_t1 WHERE kb_id = 'kb-1' "
        "AND chunk_data #>> '{dept}' = 'update' LIMIT 128"
    )
    assert order_word_literal.sql == (
        "SELECT doc_id FROM ragflow_t1 WHERE chunk_data #>> '{dept}' = 'order by dept' "
        "AND kb_id = 'kb-1' ORDER BY doc_id LIMIT 128"
    )
...
def test_tc_sql_401_validator_allows_single_key_jsonb_text_path():
    validated = _validator(field_map={"amount": "number"}).validate_and_patch(
        "SELECT chunk_data #>> '{amount}' AS amount FROM ragflow_t1 WHERE kb_id = 'kb-1' LIMIT 10"
    )
...
def test_tc_sql_402_validator_allows_multi_key_jsonb_text_path():
    validated = _validator(field_map={"profile.name": "string"}).validate_and_patch(
        "SELECT doc_id FROM ragflow_t1 WHERE chunk_data #>> '{profile,name}' = 'Alice'"
    )

    assert validated.sql == (
        "SELECT doc_id FROM ragflow_t1 WHERE chunk_data #>> '{profile,name}' = 'Alice' "
        "AND kb_id = 'kb-1' LIMIT 128"
    )
    assert validated.columns == ["doc_id"]
    assert validated.is_aggregation is False
...
def test_tc_sql_404_validator_allows_jsonb_object_extraction_operator():
    validated = _validator(field_map={"profile": "json"}).validate_and_patch(
        "SELECT doc_id FROM ragflow_t1 WHERE chunk_data #> '{profile}' "
        "@> '{\"name\": \"Alice\"}'::jsonb"
    )

    assert validated.sql == (
        "SELECT doc_id FROM ragflow_t1 WHERE chunk_data #> '{profile}' "
        "@> CAST('{\"name\": \"Alice\"}' AS JSONB) AND kb_id = 'kb-1' LIMIT 128"
    )
    assert validated.columns == ["doc_id"]
    assert validated.is_aggregation is False
...
def test_tc_sql_405_validator_allows_numeric_cast_on_exposed_jsonb_field():
    validated = _validator(field_map={"amount": "number"}).validate_and_patch(
        "SELECT doc_id FROM ragflow_t1 WHERE "
        "CAST(chunk_data #>> '{amount}' AS DOUBLE PRECISION) > 100"
    )

    assert validated.sql == (
        "SELECT doc_id FROM ragflow_t1 WHERE CAST(chunk_data #>> '{amount}' AS DOUBLE PRECISION) > 100 "
        "AND kb_id = 'kb-1' LIMIT 128"
    )
    assert validated.columns == ["doc_id"]
    assert validated.is_aggregation is False
...
def test_tc_sql_406_validator_allows_to_date_on_exposed_jsonb_field():
    validated = _validator(field_map={"dt": "date"}).validate_and_patch(
        "SELECT doc_id FROM ragflow_t1 WHERE "
        "to_date(chunk_data #>> '{dt}', 'YYYY-MM-DD') > '2026-01-01'"
    )

    assert validated.sql == (
        "SELECT doc_id FROM ragflow_t1 WHERE TO_DATE(chunk_data #>> '{dt}', 'YYYY-MM-DD') > '2026-01-01' "
        "AND kb_id = 'kb-1' LIMIT 128"
    )
    assert validated.columns == ["doc_id"]
    assert validated.is_aggregation is False
...
def test_tc_sql_407_validator_allows_jsonb_text_null_checks():
    validator = _validator(field_map={"status": "string"})
    is_null = validator.validate_and_patch(
        "SELECT doc_id FROM ragflow_t1 WHERE (chunk_data #>> '{status}') IS NULL"
    )
    is_not_null = validator.validate_and_patch(
        "SELECT doc_id FROM ragflow_t1 WHERE (chunk_data #>> '{status}') IS NOT NULL"
    )
    ...
    assert is_not_null.sql == (
        "SELECT doc_id FROM ragflow_t1 WHERE NOT (chunk_data #>> '{status}') IS NULL "
        "AND kb_id = 'kb-1' LIMIT 128"
    )
...
def test_tc_sql_409_validator_rejects_bare_or_non_gaussdb_chunk_data_access():
    scenarios = {
        "SELECT doc_id, chunk_data FROM ragflow_t1 WHERE kb_id = 'kb-1'": (
            "chunk_data may only be accessed through #> / #>>"
        ),
        "SELECT chunk_data ->> 'amount' FROM ragflow_t1": (
            "only GaussDB #> / #>> JSONB operators are allowed"
        ),
        "SELECT chunk_data['amount'] FROM ragflow_t1": (
            "chunk_data may only be accessed through #> / #>>"
        ),
        "SELECT chunk_data @> '{\"amount\":1}'::jsonb FROM ragflow_t1": (
            "chunk_data may only be accessed through #> / #>>"
        ),
        "SELECT chunk_data ? 'amount' FROM ragflow_t1": (
            "chunk_data may only be accessed through #> / #>>"
...
def test_tc_sql_410_validator_allows_jsonb_null_literal_comparison():
    sql = (
        "SELECT doc_id FROM ragflow_t1 WHERE kb_id = 'kb-1' "
        "AND chunk_data #> '{status}' = 'null'::jsonb"
    )
    ...
    assert validated.sql == (
        "SELECT doc_id FROM ragflow_t1 WHERE kb_id = 'kb-1' "
        "AND chunk_data #> '{status}' = CAST('null' AS JSONB) LIMIT 128"
    )
    assert validated.columns == ["doc_id"]
    assert validated.is_aggregation is False
...
def test_tc_sql_512_runtime_readonly_guard_enforces_table_and_preserves_safe_scopes():
    ...
    cte_alias = validator.validate_and_patch(
        "WITH rows AS (SELECT chunk_data #>> '{amount}' AS amount, kb_id "
        "FROM ragflow_t1 WHERE kb_id = 'kb-1') SELECT amount FROM rows"
    )
    predicate_before_boundary = validator.validate_and_patch(
        "SELECT doc_id FROM ragflow_t1 WHERE doc_id = 'doc1' AND kb_id = 'kb-1'"
    )
    ...
    assert cte_alias.sql == (
        'WITH rows AS (SELECT chunk_data #>> \'{amount}\' AS amount, kb_id FROM "tenant_schema".ragflow_t1 '
        "WHERE kb_id = 'kb-1') SELECT amount FROM rows LIMIT 128"
    )
    assert cte_alias.columns == ["amount"]
    assert cte_alias.is_aggregation is False
    assert predicate_before_boundary.sql == (
        'SELECT doc_id FROM "tenant_schema".ragflow_t1 '
        "WHERE doc_id = 'doc1' AND kb_id = 'kb-1' LIMIT 128"
    )
...
def test_tc_sql_705_validator_detects_all_supported_aggregations_and_non_aggregate():
    ...
    scenarios = {
        ...
        "SELECT SUM(CAST(chunk_data #>> '{amount}' AS DOUBLE PRECISION)) AS total FROM ragflow_t1": (
            "SELECT SUM(CAST(chunk_data #>> '{amount}' AS DOUBLE PRECISION)) AS total "
            "FROM ragflow_t1 WHERE kb_id = 'kb-1' LIMIT 128",
            ["total"],
            True,
        ),
        "SELECT AVG(CAST(chunk_data #>> '{amount}' AS DOUBLE PRECISION)) AS avg_amt FROM ragflow_t1": (
            "SELECT AVG(CAST(chunk_data #>> '{amount}' AS DOUBLE PRECISION)) AS avg_amt "
            "FROM ragflow_t1 WHERE kb_id = 'kb-1' LIMIT 128",
            ["avg_amt"],
            True,
        ),
        "SELECT MAX(CAST(chunk_data #>> '{amount}' AS DOUBLE PRECISION)) AS max_amt FROM ragflow_t1": (
            "SELECT MAX(CAST(chunk_data #>> '{amount}' AS DOUBLE PRECISION)) AS max_amt "
            "FROM ragflow_t1 WHERE kb_id = 'kb-1' LIMIT 128",
            ["max_amt"],
            True,
        ),
        "SELECT MIN(CAST(chunk_data #>> '{amount}' AS DOUBLE PRECISION)) AS min_amt FROM ragflow_t1": (
            "SELECT MIN(CAST(chunk_data #>> '{amount}' AS DOUBLE PRECISION)) AS min_amt "
            "FROM ragflow_t1 WHERE kb_id = 'kb-1' LIMIT 128",
            ["min_amt"],
            True,
        ),
...
def test_tc_sql_706_validator_returns_selected_column_names_and_aliases():
    validator = _validator(field_map={"amount": "number"})
    aliased = validator.validate_and_patch(
        "SELECT doc_id, chunk_data #>> '{amount}' AS amt FROM ragflow_t1"
    )
...
def test_tc_sql_707_select_alias_is_allowed_in_group_and_order_by_for_both_guards():
    sql = (
        "SELECT chunk_data #>> '{dept}' AS dept, COUNT(*) AS cnt FROM ragflow_t1 "
        "WHERE kb_id = 'kb-1' GROUP BY dept ORDER BY dept"
    )
    ...
    expected_sql = (
        "SELECT chunk_data #>> '{dept}' AS dept, COUNT(*) AS cnt FROM ragflow_t1 "
        "WHERE kb_id = 'kb-1' GROUP BY dept ORDER BY dept LIMIT 128"
    )
