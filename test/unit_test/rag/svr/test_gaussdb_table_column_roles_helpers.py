...
def test_tc_sql_009_parser_config_merge_and_metadata_cleanup_keys():
    ...
    assert table_parser_strip_doc_metadata_keys(
        {"table_column_names": ["Region", " SKU "]}
    ) == frozenset({"Region", "SKU"})
    assert table_parser_strip_doc_metadata_keys(
        {"table_column_roles": {"x": "metadata", "y": "indexing"}}
    ) == frozenset({"x", "y"})
    assert table_parser_strip_doc_metadata_keys(
        {"table_column_names": [], "table_column_roles": {"only": "both"}}
    ) == frozenset({"only"})
