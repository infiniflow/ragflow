...
# keyed by the original column name (line 609 in rag/app/table.py:
# `stored[str(col_name)] = row[col_name]`). The SQL prompt the LLM sees
# at api/db/services/dialog_service.py:1125+ enumerates `field_map`
# keys, but the SQL examples reference those exact keys via
# `json_extract_string(chunk_data, '$.FieldName')`. Before this fix the
# keys were the pinyin form (`py_clmns[i].lower()`), which silently
# broke non-ASCII columns because the LLM was told `xing_ming` but
# `chunk_data` was keyed by `姓名`. The fix keys `field_map` by the
# original (replaced underscore) column name so both sides match.
