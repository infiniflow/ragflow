...
class _FakeAsanaConnector:
    instances = []

    def __init__(self, workspace_id, project_ids=None, team_id=None):
        ...
        self.poll_source_call = None
        self.retrieve_all_slim_docs_perm_sync_called = False
        _FakeAsanaConnector.instances.append(self)
    ...
    def retrieve_all_slim_docs_perm_sync(self):
        ...
        return iter(
            (
                [types.SimpleNamespace(id="asana:task-1:att-1")],
                [types.SimpleNamespace(id="asana:task-2:att-2")],
            )
        )
...
@pytest.mark.anyio
@pytest.mark.p2
async def test_asana_generate_returns_snapshot_when_sync_deleted_enabled(monkeypatch):
    _FakeAsanaConnector.instances = []
    monkeypatch.setattr(sync_data_source, "AsanaConnector", _FakeAsanaConnector)
    poll_start = datetime(2026, 1, 1, tzinfo=timezone.utc)
    task = {**_make_task(), "reindex": "0", "poll_range_start": poll_start, "skip_connection_log": True}

    result = await sync_data_source.Asana(
        {
            "asana_workspace_id": "workspace-1",
            "asana_project_ids": "project-1",
            "asana_team_id": "team-1",
            "sync_deleted_files": True,
            "credentials": {"asana_api_token_secret": "token-1"},
        }
    )._generate(task)

    document_generator, file_list = result
    connector = _FakeAsanaConnector.instances[0]

    assert list(document_generator) == [["poll-sync"]]
    assert [doc.id for doc in file_list] == ["asana:task-1:att-1", "asana:task-2:att-2"]
    assert connector.credentials == {"asana_api_token_secret": "token-1"}
    assert connector.poll_source_call[0] == poll_start.timestamp()
    assert connector.poll_source_call[1] >= connector.poll_source_call[0]
    assert connector.retrieve_all_slim_docs_perm_sync_called
    assert not connector.load_from_state_called


@pytest.mark.anyio
@pytest.mark.p2
async def test_asana_generate_skips_snapshot_for_full_reindex(monkeypatch):
    _FakeAsanaConnector.instances = []
    monkeypatch.setattr(sync_data_source, "AsanaConnector", _FakeAsanaConnector)
    task = {**_make_task(), "reindex": "1", "skip_connection_log": True}
    ...
    assert list(result) == [["full-sync"]]
    assert connector.load_from_state_called
    assert connector.poll_source_call is None
    assert not connector.retrieve_all_slim_docs_perm_sync_called
