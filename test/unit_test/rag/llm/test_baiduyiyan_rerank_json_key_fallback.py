...
"""Regression tests for the BaiduYiyanRerank (Baidu Qianfan rerank) ``key``
JSON-decode fallback (#17660).

Pre-fix, ``BaiduYiyanRerank.__init__`` did::
...
The fix wires ``BaiduYiyanRerank.__init__`` through the existing
``_resolve_qianfan_credentials`` helper in :mod:`rag.llm.key_utils`
(the same helper PR #17390 added for the chat side and PR #17505 added
for the embed side). The helper:
...
The rerank class then continues with the existing
``key.get("yiyan_ak", "")`` / ``key.get("yiyan_sk", "")`` calls --
the field-presence check is unchanged.

This is the rerank-side companion of PR #17390 (chat side) and
PR #17505 (embed side). The helper itself is the same; only the call
site changes. Test file is named
``test_baiduyiyan_rerank_json_key_fallback.py`` (not the chat-side
``test_qianfan_json_key_fallback.py`` or the embed-side
``test_baiduyiyan_embed_json_key_fallback.py``) to avoid pytest
collection conflicts if multiple PRs land in the same release.
"""
...
# --------------------------------------------------------------------------- #
# 2. End-to-end: BaiduYiyanRerank.__init__ uses the helper, not bare json.loads
# --------------------------------------------------------------------------- #
...
def _install_qianfan_reranker_stub(monkeypatch):
    """Stub the ``qianfan.resources.Reranker`` so
    ``BaiduYiyanRerank.__init__`` can run without the real optional
    ...
    the ``qianfan.resources.Reranker`` symbol -- the helper test
    exercises the ModelException path before the SDK is even touched,
    so for the happy-path e2e we return a MagicMock that captures the
    constructor kwargs (``ak``, ``sk``, ``request_timeout``) for
    assertion.
    """
    qianfan = MagicMock()
    qianfan_resources = MagicMock()
    captured = {"reranker_kwargs": None}

    def _reranker_factory(*args, **kwargs):
        captured["reranker_kwargs"] = kwargs
        return MagicMock()

    qianfan_resources.Reranker = MagicMock(side_effect=_reranker_factory)
    qianfan.resources = qianfan_resources
    monkeypatch.setitem(sys.modules, "qianfan", qianfan)
    monkeypatch.setitem(sys.modules, "qianfan.resources", qianfan_resources)
    return captured


@pytest.mark.p0
class TestBaiduYiyanRerankCallSite:
    def test_json_dict_constructs_with_parsed_fields(self, monkeypatch):
        ...
        BaiduYiyanRerank.__init__ completes (with the qianfan SDK
        stubbed). The stubbed Reranker is called with the parsed
        ``yiyan_ak`` and ``yiyan_sk``."""
        from rag.llm.rerank_model import BaiduYiyanRerank

        captured = _install_qianfan_reranker_stub(monkeypatch)
        BaiduYiyanRerank(KEY_VALID, model_name="bce-reranker-base_v1")
        # The qianfan SDK was called with the parsed ak + sk + request_timeout.
        called_kwargs = captured["reranker_kwargs"]
        assert called_kwargs["ak"] == "ak_test"
        assert called_kwargs["sk"] == "sk_test"
        assert called_kwargs["request_timeout"] == 30
    ...
    def test_json_array_raises_model_exception(self, monkeypatch):
        ...
        _install_qianfan_reranker_stub(monkeypatch)
        with pytest.raises(ModelException) as exc_info:
            BaiduYiyanRerank(KEY_JSON_ARRAY, model_name="bce-reranker-base_v1")
        msg = str(exc_info.value)
        assert "BaiduYiyan" in msg
        assert "object" in msg
        assert "list" in msg
    ...
    def test_json_number_raises_model_exception(self, monkeypatch):
        ...
        _install_qianfan_reranker_stub(monkeypatch)
        with pytest.raises(ModelException) as exc_info:
            BaiduYiyanRerank(KEY_JSON_NUMBER, model_name="bce-reranker-base_v1")
    ...
    def test_plain_baidu_api_key_raises_model_exception(self, monkeypatch):
        ...
        _install_qianfan_reranker_stub(monkeypatch)
        with pytest.raises(ModelException) as exc_info:
            BaiduYiyanRerank("bce-v3/ALTAK-EXAMPLE/EXAMPLE", model_name="bce-reranker-base_v1")
        assert "BaiduYiyan" in str(exc_info.value)
    ...
    def test_json_dict_missing_yiyan_ak_uses_empty_string(self, monkeypatch):
        ...
        captured = _install_qianfan_reranker_stub(monkeypatch)
        BaiduYiyanRerank(json.dumps({"yiyan_sk": "sk_test"}), model_name="bce-reranker-base_v1")
        called_kwargs = captured["reranker_kwargs"]
