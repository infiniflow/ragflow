...
def _load_memory_routes_module(monkeypatch):
    ...
    constants_mod.RetCode = SimpleNamespace(SUCCESS=0, ARGUMENT_ERROR=101, SERVER_ERROR=100, NOT_FOUND=104)
    monkeypatch.setitem(sys.modules, "common.constants", constants_mod)
    exceptions_mod = ModuleType("common.exceptions")
    exceptions_mod.ArgumentException = type("ArgumentException", (Exception,), {})
    exceptions_mod.NotFoundException = type("NotFoundException", (Exception,), {})
    monkeypatch.setitem(sys.modules, "common.exceptions", exceptions_mod)

    api_utils_mod = ModuleType("api.utils.api_utils")
    api_utils_mod.validate_request = lambda *_a, **_k: lambda f: f
    api_utils_mod.get_request_json = lambda: _AwaitableValue({})
    api_utils_mod.get_error_argument_result = lambda message: {"code": 101, "message": message, "data": None}
    api_utils_mod.get_json_result = lambda **kw: {"code": kw.get("code", 0), "message": kw.get("message", ""), "data": kw.get("data")}
    monkeypatch.setitem(sys.modules, "api.utils.api_utils", api_utils_mod)
    pagination_mod = ModuleType("api.utils.pagination_utils")
    pagination_mod.DEFAULT_PAGE = 1
    pagination_mod.DEFAULT_PAGE_SIZE = 30
    pagination_mod.validate_rest_api_ids = lambda ids, _name: ids
    pagination_mod.validate_rest_api_page = lambda page: int(page)
    pagination_mod.validate_rest_api_page_size = lambda size: int(size) if 1 <= int(size) <= 100 else (_ for _ in ()).throw(ValueError("page_size must be less than or equal to 100"))
    monkeypatch.setitem(sys.modules, "api.utils.pagination_utils", pagination_mod)
    tenant_mod = ModuleType("api.db.joint_services.tenant_model_service")
    tenant_mod.ensure_tenant_model_ids_for_params = lambda *_a, **_k: None
    monkeypatch.setitem(sys.modules, "api.db.joint_services.tenant_model_service", tenant_mod)
...
def test_search_message_rejects_non_numeric_weights(monkeypatch, field):
    module = _load_memory_routes_module(monkeypatch)
    monkeypatch.setattr(module, "request", SimpleNamespace(args=_DummyArgs({"memory_id": "m1", "query": "hello", field: "bad"})))
    res = _run(inspect.unwrap(module.search_message)())
    assert res["code"] == module.RetCode.ARGUMENT_ERROR, res
