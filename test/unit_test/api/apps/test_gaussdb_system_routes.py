...
@pytest.mark.p1
def test_tc_cfg_703_gaussdb_status_route_returns_500_when_probe_raises(monkeypatch):
    module = _load_system_module(monkeypatch)

    def raise_probe_error():
        raise RuntimeError(
            f'probe failed password="{ROUTE_PASSWORD_SENTINEL}" '
            f"dsn={ROUTE_DSN_SENTINEL} access_token='{ROUTE_TOKEN_SENTINEL}'"
        )
