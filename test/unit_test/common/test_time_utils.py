...
class TestTimestampToDate:
    ...
    def test_basic_timestamp_conversion(self):
        ...
        # Calculate expected value based on local timezone
    ...
    def test_custom_format_string(self):
        ...
        # Test different format strings - calculate expected values dynamically
        result1 = timestamp_to_date(timestamp, "%Y-%m-%d")
        expected1 = time.strftime("%Y-%m-%d", time.localtime(timestamp / 1000))
        assert result1 == expected1

        result2 = timestamp_to_date(timestamp, "%H:%M:%S")
        expected2 = time.strftime("%H:%M:%S", time.localtime(timestamp / 1000))
        assert result2 == expected2

        result3 = timestamp_to_date(timestamp, "%Y/%m/%d %H:%M")
        expected3 = time.strftime("%Y/%m/%d %H:%M", time.localtime(timestamp / 1000))
    ...
    def test_millisecond_precision(self):
        ...
        # Should truncate milliseconds - calculate expected time
        expected_time = time.strftime("%H:%M:%S", time.localtime(timestamp / 1000))
        assert expected_time in result

    def test_various_timestamps(self):
        """Test conversion with various timestamp values"""
        test_cases = [
            1609459200000,  # 2020-12-31 16:00:00 UTC
            2524608000000,  # 2050-01-01 Future date (safer range)
        ]

        for timestamp in test_cases:
            result = timestamp_to_date(timestamp)
            # Calculate expected value dynamically
            expected = time.strftime("%Y-%m-%d", time.localtime(timestamp / 1000))
...
class TestDeltaSeconds:
    ...
    def test_none_input(self):
        """Test that None input raises TypeError"""
        with pytest.raises(TypeError):
            delta_seconds(None)  # type: ignore
