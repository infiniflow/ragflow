...
def test_doc_engine_old_yaml():
    return_value = {
        ...
        "infinity": {"uri": "127.0.0.1:23817", "db_name": "old_db"},
    }
    with patch("core.config.app.load_yaml", return_value=return_value):
        config = AppConfig()

    # Elasticsearch
    es_cfg = config.doc_engine.es
...
def test_doc_engine_new_yaml():
    return_value = {
        "doc_engine": {
            ...
            "infinity": {"uri": "127.0.0.2:23818", "db_name": "new_db"},
        }
    }
    with patch("core.config.app.load_yaml", return_value=return_value):
        config = AppConfig()
