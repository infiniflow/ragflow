...
class TestUserDeletion:
    ...
    @pytest.mark.p2
    def test_delete_user_removes_storage_files(self, admin_session, tmp_path, storage_impl):
        ...
        try:
            ...
            assert not storage_impl.bucket_exists(dataset_id_1), f"Storage should not contain bucket for dataset {dataset_id_1} after deletion"

            # verify user 2's data did not get deleted
            assert storage_impl.bucket_exists(dataset_id_2), f"Data of user 2 was deleted. Storage should have bucket for dataset {dataset_id_2}"
            assert storage_impl.obj_exist(dataset_id_2, filename_2), f"Data of user 2 was deleted. Storage should have file {filename_2} in bucket of dataset {dataset_id_2}"
        finally:
            # cleanup
            if username_1:
                change_user_activation(admin_session, username_1, False)
                delete_user(admin_session, username_1)
    ...
    def _create_user_with_data(admin_session, tmp_path) -> tuple[str, dict[str, str], str, str]:
        ...
        assert create_dataset_response["code"] == 0, create_dataset_response
        assert isinstance(create_dataset_response["data"], dict), create_dataset_response
