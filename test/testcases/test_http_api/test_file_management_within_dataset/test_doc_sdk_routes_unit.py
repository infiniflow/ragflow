...
def _load_document_download_module(monkeypatch):
    repo_root = Path(__file__).resolve().parents[4]
    apps_mod = ModuleType("api.apps")
    apps_mod.current_user = SimpleNamespace(id="tenant-1")
    apps_mod.login_required = lambda func: func
    ...
    apps_services_mod = ModuleType("api.apps.services")
    apps_services_mod.__path__ = [str(repo_root / "api" / "apps" / "services")]
    monkeypatch.setitem(sys.modules, "api.apps.services", apps_services_mod)
    ...
    document_api_service_mod.map_doc_keys_with_run_status = lambda doc, run_status="0": doc
    ...
    services_pkg = ModuleType("api.db.services")
    services_pkg.__path__ = [str(repo_root / "api" / "db" / "services")]
    services_pkg.duplicate_name = lambda query_func, **kwargs: kwargs.get("name", "file")
    monkeypatch.setitem(sys.modules, "api.db.services", services_pkg)
    ...
    knowledgebase_service_mod = ModuleType("api.db.services.knowledgebase_service")
    knowledgebase_service_mod.KnowledgebaseService = SimpleNamespace(accessible=lambda **_kwargs: False)
    monkeypatch.setitem(sys.modules, "api.db.services.knowledgebase_service", knowledgebase_service_mod)
    ...
    check_team_permission_mod.check_kb_team_permission = lambda *_args, **_kwargs: None
    ...
    api_utils_mod.get_data_error_result = lambda message="": {"message": message}
    ...
    validation_utils_mod.format_validation_error_message = lambda e: str(e)
    ...
    common_settings_mod = ModuleType("common.settings")
    common_settings_mod.STORAGE_IMPL = SimpleNamespace(get=lambda *_args, **_kwargs: b"")
    monkeypatch.setitem(sys.modules, "common.settings", common_settings_mod)
    ...
    spec = importlib.util.spec_from_file_location("test_doc_sdk_download_unit", module_path)
    module = importlib.util.module_from_spec(spec)
    module.manager = _DummyManager()
    spec.loader.exec_module(module)
    return module
...
class TestDocRoutesUnit:
    ...
    def test_download_and_download_doc_errors(self, monkeypatch):
        module = _load_document_download_module(monkeypatch)
