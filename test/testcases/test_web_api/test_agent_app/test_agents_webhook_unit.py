...
def test_agents_crud_unit_branches(monkeypatch):
    ...
    monkeypatch.setattr(
        module,
        "request",
        SimpleNamespace(args={"owner_ids": "unauthorized", "desc": "false", "page": "1", "page_size": "10"}),
    )
    monkeypatch.setattr(
    ...
    res = inspect.unwrap(module.list_agents)("tenant-1")
    assert res["code"] == module.RetCode.OPERATING_ERROR
    assert "authorized owner_ids" in res["message"]

    captured = {}

    def fake_get_list(_tenant_ids, _tenant_id, _page, _page_size, _orderby, desc, *_rest):
        captured["desc"] = desc
        return [{"id": "agent-1"}], 1
    ...
    monkeypatch.setattr(module.UserCanvasService, "get_by_tenant_ids", fake_get_list)
    ...
    res = _run(inspect.unwrap(module.create_agent)("tenant-1"))
    ...
    res = _run(inspect.unwrap(module.update_agent)("agent-1", "tenant-1"))
    assert res["code"] == module.RetCode.OPERATING_ERROR

    calls = {"update": 0, "save_or_replace_latest": 0}
    monkeypatch.setattr(module.UserCanvasService, "query", lambda **_kwargs: True)
    monkeypatch.setattr(module.UserCanvasService, "get_by_id", lambda _agent_id: (True, SimpleNamespace(title="agent-1", canvas_category=module.CanvasCategory.Agent)))
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(module.UserCanvasService, "query", lambda **_kwargs: False)
    res = inspect.unwrap(module.delete_agent)("agent-1", "tenant-1")
    assert res["code"] == module.RetCode.OPERATING_ERROR
