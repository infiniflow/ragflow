...
The service listens on port **8124** by default. Pass `--port` to change it:

```bash
python deepdoc/servers/deepdoc_svr.py --port 9000 --model-dir /path/to/models
```
...
```
curl -X POST http://localhost:8124/predict/dla \
...
```
curl -X POST http://localhost:8124/predict/tsr \
...
```
curl -X POST "http://localhost:8124/predict/ocr" \
  -F "operator=det" \
  -F "request=@page.jpg;type=image/jpeg"
```
