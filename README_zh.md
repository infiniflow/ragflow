...
> 🎁 **[Atlas Cloud](https://www.atlascloud.ai/?utm_source=github&utm_medium=link&utm_campaign=ragflow)** 是一个全模态、兼容 OpenAI 接口的 AI 推理平台。RAGFlow 可以把它作为即插即用的对话**模型供应商** —— 在模型设置里选择 **Atlas**，即可通过单一 API 访问 DeepSeek、Qwen、GLM、Kimi、MiniMax、Claude、GPT、Gemini 等众多模型，无需对接多家厂商。高性价比：[coding plan](https://www.atlascloud.ai/console/coding-plan)。

```env
# 选择 “Atlas” 供应商时，Base URL 已预设为 https://api.atlascloud.ai/v1
ATLAS_API_KEY=<atlascloud-api-key>
# 推荐的默认推理模型：
ATLAS_MODEL=deepseek-ai/deepseek-v4-pro
```

`deepseek-ai/deepseek-v4-pro` 是带思维链的推理模型，调用时请给足 `max_tokens`（建议 >= 512），否则返回可能被截断、content 为空。
