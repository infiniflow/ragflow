...
def main() -> int:
    ...
    try:
        ...
        log.info("Probing mock server /openapi.json")
        probe = requests.head(f"{base_url}/openapi.json", timeout=5)
        if probe.status_code not in (200, 301, 302, 307, 308):
            log.error("Mock server probe failed with status %s", probe.status_code)
            return 1
        ...
        with tempfile.TemporaryDirectory(prefix="monkeyocr_e2e_") as tmp_dir:
            tmp = Path(tmp_dir)
            log.info("Using temporary workspace %s", tmp)
            ...
            log.info("Wrote sample PDF to %s", pdf_path)

            parser = MonkeyOCRParser(monkeyocr_api=base_url)
            log.info("Checking MonkeyOCRParser installation")
            ok, reason = parser.check_installation()
            if not ok:
                log.error("check_installation failed: %s", reason)
                return 1

            log.info("Parsing PDF via MonkeyOCRParser")
            ...
            if not any(_Handler.BODY in t for t in texts):
                log.error("MonkeyOCRParser sections missing body: %r", texts)
                return 1

            model = MonkeyOCROcrModel(
                {
                    "monkeyocr_apiserver": base_url,
                    "monkeyocr_output_dir": str(tmp / "model-out"),
                    "monkeyocr_delete_output": 0,
                },
                "monkeyocr-test",
            )
            log.info("Parsing PDF via MonkeyOCROcrModel")
            ...
            if not any(_Handler.BODY in t for t in texts2):
                log.error("MonkeyOCROcrModel sections missing body: %r", texts2)
                return 1

        log.info("OK: MonkeyOCR HTTP integration (parser + OCR model)")
        print("OK: MonkeyOCR HTTP integration (parser + OCR model)")
        return 0
    finally:
        log.info("Shutting down mock MonkeyOCR server")
        server.shutdown()
