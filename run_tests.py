...
class TestRunner:
    ...
    def print_info(message: str) -> None:
        ...
        print(f"{set_color(s='[INFO]', color='blue')} {message}", flush=True)

    @staticmethod
    def print_error(message: str) -> None:
        """Print error message"""
        print(f"{set_color(s='[ERROR]', color='red')} {message}", flush=True)
...
"""
    ...
    def run_tests(self) -> bool:
        ...
        # Optionally run pytest under `python -v` to trace every import. This is
        # the most reliable way to see where collection hangs (importing heavy
        # modules like ragflow/deepdoc). Toggle with RAGFLOW_TEST_IMPORT_TRACE=1.
        final_cmd = cmd
        if os.environ.get("RAGFLOW_TEST_IMPORT_TRACE") == "1":
            final_cmd = [self.python, "-v", "-m", "pytest", *cmd[1:]]

        print(f"\n{set_color(s='[EXECUTING]', color='blue')} {' '.join(final_cmd)}\n", flush=True)
        self.print_info("Launching pytest — collection of test modules may take a while...")
