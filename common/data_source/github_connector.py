...
from common.data_source.models import ExternalAccess, Document, DocumentFailure, TextSection


# NOTE:
# This connector was originally adapted from Onyx. Ragflow OSS doesn't vendor Onyx's
# supplemental GitHub modules, so we keep this file self-contained.

# Optional GitHub Enterprise base URL. Empty means use api.github.com.
GITHUB_CONNECTOR_BASE_URL = ""


class SerializedRepository(BaseModel):
    """Serialized GitHub repo for checkpointing.

    We keep enough information to (optionally) reconstruct a Repository object
    without re-fetching, but we also provide a safe fallback by repo id.
    """

    # id is part of raw_data as well, but pulled out for convenience
    id: int
    headers: dict[str, Any] | None = None
    raw_data: dict[str, Any] | None = None

    def to_Repository(self, github_client: Github) -> Repository.Repository:
        """Best-effort conversion back to a PyGithub Repository.

        If we can't access the internal requester or don't have enough data,
        fall back to a normal API fetch by repo id.
        """

        if not self.headers or not self.raw_data:
            return github_client.get_repo(self.id)

        # PyGithub stores a Requester on the Github instance, but the private
        # attribute name varies across versions.
        ...
        try:
            return Repository.Repository(
                requester,
                cast(dict[str, Any], self.headers),
                cast(dict[str, Any], self.raw_data),
                completed=True,
            )
        except Exception:
            return github_client.get_repo(self.id)
...
def deserialize_repository(
    serialized_repo: SerializedRepository, github_client: Github
) -> Repository.Repository:
    """Re-hydrate a Repository from checkpoint (best-effort)."""

    return serialized_repo.to_Repository(github_client)


def sleep_after_rate_limit_exception(github_client: Github) -> None:
    """Best-effort sleep until GitHub rate limit reset."""

    try:
        reset_dt = github_client.get_rate_limit().core.reset
        sleep_seconds = max(
            1,
            int((reset_dt.replace(tzinfo=timezone.utc) - datetime.now(timezone.utc)).total_seconds())
            + 5,
        )
    except Exception:
        sleep_seconds = 60

    logging.warning("Ran into GitHub rate-limit. Sleeping %s seconds.", sleep_seconds)
    time.sleep(sleep_seconds)


def get_external_access_permission(
    repo: Repository.Repository, github_client: Github
) -> ExternalAccess | None:
    """Placeholder for permission sync.

    Ragflow OSS doesn't currently implement GitHub permission sync.
    Return None to indicate 'no external access data'.
    """
...
if __name__ == "__main__":
    ...
    # Minimal demo runner (kept self-contained; avoids any external ConnectorRunner dependency)
        ...
        try:
            while True:
                ...
                if isinstance(item, ConnectorFailure):
                    print(f"Failure: {item.failure_message}")
                else:
                    print(f"Document: {item.semantic_identifier}")
