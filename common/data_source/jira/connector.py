...
class JiraConnector(CheckpointedConnectorWithPermSync, SlimConnectorWithPermSync):
    ...
    def _load_with_retry(
        ...
        while True:
            ...
            logger.info(
                "[Jira] Executing Jira JQL attempt %s (buffered_retry=%s)",
                attempt,
                retried_with_buffer,
            )
            logger.debug(
                "[Jira] JQL time range: start=%s, adjusted_start=%s, end=%s, overlap_buffer_s=%s",
                attempt_start,
                adjusted_start,
                end,
            ...
            try:
                ...
                if attempt_start is not None and not retried_with_buffer and is_atlassian_date_error(exc):
                    ...
                    logger.info("[Jira] Atlassian date error detected; retrying with buffered start time.")
                    logger.debug("[Jira] Retry start time: %s", attempt_start)
                    continue
                raise
