...
# Frontend display names (lowercased) → canonical names whose .lower() matches PARSERS keys.
# E.g. "TCADP Parser" → "TCADP" → .lower() → "tcadp" ✓
#      "Plain Text"   → "PlainText" → .lower() → "plaintext" ✓
...
def normalize_layout_recognizer(layout_recognizer_raw: Any) -> tuple[Any, str | None]:
    ...
    if isinstance(layout_recognizer_raw, str):
        lowered = layout_recognizer_raw.strip().lower()
        ...
        elif lowered in _DISPLAY_NAME_MAP:
            layout_recognizer = _DISPLAY_NAME_MAP[lowered]
