...
def timestamp_to_date(timestamp, format_string="%Y-%m-%d %H:%M:%S"):
    """
    Convert a timestamp to formatted UTC date string.
...
def date_string_to_timestamp(time_str, format_string="%Y-%m-%d %H:%M:%S"):
    """
    Convert a UTC date string to timestamp in milliseconds.

    Args:
        time_str: UTC date string to convert
    ...
    time_stamp = int(calendar.timegm(time_array) * 1000)
    return time_stamp

def datetime_format(date_time: datetime.datetime) -> datetime.datetime:
    """
    Normalize a datetime object by removing microsecond component and converting to UTC.

    Creates a new datetime object with only year, month, day, hour, minute, second.
    Microseconds are set to 0. If the input has timezone info, it is converted to UTC.

    Args:
        date_time: datetime object to normalize

    Returns:
        datetime.datetime: New naive datetime object in UTC without microseconds
    ...
    if date_time.tzinfo is not None:
        date_time = date_time.astimezone(datetime.timezone.utc).replace(tzinfo=None)
...
def get_format_time() -> datetime.datetime:
    """
    Get current UTC datetime normalized without microseconds.

    Returns:
        datetime.datetime: Current UTC datetime with microseconds set to 0

    Example:
        >>> get_format_time()
        datetime.datetime(2024, 1, 1, 12, 30, 45)
    """
    return datetime_format(datetime.datetime.utcnow())


def delta_seconds(date_string: str):
    """
    Calculate seconds elapsed from a given date string to now.
    Both the input and current time are treated as naive UTC datetimes.
