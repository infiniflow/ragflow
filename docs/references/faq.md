...
2. If your network works fine, the issue is with the Docker network configuration. Replace the Docker building command:
...
This means your MySQL container failed to start. Try replacing `mysql:5.7.18` with `mariadb:10.5.8` in **docker-compose-base.yml**.
...
#### 4.13 Why did I fail to upload a 128MB+ file to my locally deployed RAGFlow?
...
You can use Ollama, Xinference, IPEX-LLM, or jina to deploy local LLM. See [here](../guides/deploy_local_llm.mdx) for more information.

### How to interconnect RAGFlow with Ollama servers?
