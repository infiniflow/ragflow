...
If the **Begin** component takes parameters, pass them in `"inputs"`:
...
- `"query"`: (*Body parameter*), `string`, *Required*  
  The user message for the agent. This field replaces the old `"question"` field.
- `"stream"`: (*Body parameter*), `boolean`  
  Indicates whether to output responses in a streaming way:
  - `true`: Enable streaming.
  - `false`: Disable streaming.
- `"session_id"`: (*Body parameter*), `string`  
  The session ID. If omitted, a new session is generated.
- `"inputs"`: (*Body parameter*), `object`  
  Variables defined in the **Begin** component.
- `"files"`: (*Body parameter*), `list[object]`  
  Optional file references already uploaded for this agent.
- `"user_id"`: (*Body parameter*), `string`  
  Optional user-defined ID. Valid only when `session_id` is not provided.
- `"return_trace"`: (*Body parameter*), `boolean`  
  Whether to include execution trace logs in `node_finished` events or the non-stream response.
- `"release"`: (*Body parameter*), `boolean`  
  Whether to use the latest published version of the agent.

:::tip NOTE
For now, this method does not support a file type Begin variable directly. As a workaround, upload the file first through `http://{address}/v1/canvas/upload/{agent_id}` and use the returned file ID.
:::
