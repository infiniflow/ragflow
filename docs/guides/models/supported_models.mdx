...
OpenAI-compatible speech-to-text services that expose `/v1/audio/transcriptions` can also be configured through **OpenAI-API-Compatible** with the **ASR** model type. The base URL must use HTTPS, except for trusted local endpoints such as `http://localhost:8000/v1`.
