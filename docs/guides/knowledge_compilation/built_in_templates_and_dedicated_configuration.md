...
Defines the system prompt used to extract atomic claims from leaf chunks before clustering.

The extracted claims include supporting evidence from the source chunks and are used to represent the semantic information of the chunks during clustering. This helps reduce the influence of layout or other irrelevant text when grouping related content.

Leave this field empty to use the built-in claim extraction rules. Customize it only when you need domain-specific claim extraction requirements.
...
#### Re-chunk by Tree Leaves

Determines whether the source chunks belonging to each leaf cluster are merged into a new retrieval chunk.

When enabled, RAGFlow merges the source chunks of each leaf cluster into a single replacement chunk. The original chunks are retained but marked as unavailable for retrieval.

This allows the final retrieval chunks to follow the semantic boundaries identified by the generated Tree instead of the original chunk boundaries.

Only one Tree template in the same template group can enable this option.
...
### Re-chunk parser output

When **Re-chunk parser output** is enabled, RAGFlow reorganizes the chunks generated by the parser before running PageIndex knowledge compilation according to the configured **Rechunking rules**.

The rechunking process attempts to preserve the original content order, heading hierarchy, and section boundaries while keeping structures such as tables, lists, and code blocks intact. This helps produce input that is better suited for building the hierarchical structure used by PageIndex.

**Rechunking rules** define how parser-generated chunks are reorganized, such as whether adjacent chunks can be merged, how headings should remain associated with their content, and how section boundaries should be preserved.

When this option is disabled, PageIndex uses the original chunks generated by the parser directly for knowledge compilation.
