...
:::danger IMPORTANT
RAGFlow has implemented MinerU PDF document parsing is available since v0.22.0. RAGFlow supports MinerU (&ge; 2.6.3) as an optional PDF parser with multiple backends. RAGFlow acts only as a *remote client* for MinerU, calling the MinerU API to parse documents, reading the returned output files, and ingesting the parsed content. To use this feature:
:::
