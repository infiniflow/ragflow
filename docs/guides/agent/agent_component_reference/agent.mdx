...
From v0.20.5 onwards, four framework-level prompt blocks is available in the **System prompt** field. Type `/` or click **(x)** to view them; they appear under the **Framework** entry in the dropdown menu.
...
- `task_analysis` prompt block
  ...
  - Available *only* when this **Agent** component is acting as a lead Agent.
  - Input variables:
    ...
    - `tool_desc`: A description of the tools and sub_Agent that can be called.
...
- `plan_generation` prompt block
  ...
  - Available *only* when this **Agent** component is acting as a lead Agent.
  - Input variables:
    - `task_analisys`: The analysis result of the current task.
    - `desc`: A description of the tool or sub-Agent currently being called.
...
- `reflection` prompt block
  ...
  - Input variables:
    ...
    - `call.name`：The names of the tools called.
