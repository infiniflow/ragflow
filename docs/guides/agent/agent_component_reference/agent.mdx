...
The user-defined prompt. Defaults to `sys.query`, the user query. As a rule of thumb, if you use the **Agent** component as a standalone module (not as a planner), you usually need to specify the correspondent **Retrieval** component's output variable (`formalized_content`) here as part of the input for the LLM. 
