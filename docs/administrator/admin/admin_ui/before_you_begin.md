...
- Otherwise a random password is generated and printed **once** to the admin server log on first startup (check `docker/ragflow-logs` in a docker deployment) — retrieve it from the log, log in, and change it immediately.
