...
| Kvrocks | Provides the Redis-compatible cache used by the Go services. | Pages respond slowly, sessions become abnormal, or cached state cannot be read or updated. | System cache, session management, and features that use cached state. |
| NATS | Provides the JetStream message queue used by the Go services. | Documents remain in `Waiting for processing`, or backend tasks cannot start. | Asynchronous task scheduling for document parsing, knowledge compilation, memory processing, synchronization, Embedding, and index building. |
