...
### Get a `Request error 404: undefined` when upgrading to v0.27.1

To resolve this issue, do either of the following:

- Pull the latest source code from the [main branch](https://github.com/infiniflow/ragflow), then pull and start the v0.27.1 image.
- Update `RAGFLOW_IMAGE` from `infiniflow/ragflow:latest` to `infiniflow/ragflow:v0.27.1` in the [.env file](https://github.com/infiniflow/ragflow/blob/main/docker/.env), then restart the service.
...
These models affect different stages of the RAG pipeline:

- Changing the **embedding model** generally requires rebuilding the vector index.
- Changing the **reranker** affects how retrieved results are reordered and does not change the original documents.
- Changing the **LLM** mainly affects query understanding and answer generation and generally does not require reparsing the dataset.
