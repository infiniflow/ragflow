...
Yes, we do. Stream output is enabled by default in chat assistant and agent. Note that you cannot disable stream out from within RAGFlow's UI. To disable stream output in the responses, use RAGFlow's Python or RESTful APIs:
