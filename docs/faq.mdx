...
| `MINERU_BACKEND`       | MinerU parsing backend             | `pipeline`                          | `MINERU_BACKEND=pipeline\|vlm-auto-engine\|hybrid-auto-engine` (current) or `vlm-transformers\|vlm-vllm-engine\|vlm-mlx-engine\|vlm-vllm-async-engine\|vlm-http-client\|vlm-lmdeploy-engine` (legacy) |
