...
> Note: The MinerU parser also supports a few parser-level options that are configured in parser settings (not as environment variables). Important ones:
>
> - `mineru_batch_size` (default 30) controls pages per batch; valid values are 1..500 and values will be clamped on the server-side if out of range.
> - `mineru_strict_mode` (default true) is currently a backend-only option (requires no UI change to function). If you want to expose it in the frontend, add a boolean form field and propagate it into `parser_config` as `mineru_strict_mode`.
