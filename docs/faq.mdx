...
The current Go backend also supports [Infinity](https://github.com/infiniflow/infinity), OceanBase, and SeekDB as document engines. Infinity is an AI-native database developed by InfiniFlow and optimized for RAG workloads. Support levels and available features may vary between document engines. OpenSearch and GaussDB are not currently available as document engines in the Go backend.
...
The documented Go image build targets Linux AMD64 and supports CPU deployments and NVIDIA GPU deployments. RAGFlow does not currently publish a maintained ARM64 Go image or a complete ARM64 build procedure. Building on ARM64 requires an ARM64-compatible dependency image and native libraries, and you must also confirm that the selected document engine supports ARM64. The AMD64 command in the [quickstart guide](./quickstart.mdx#start-up-the-server) cannot be used unchanged to produce an ARM64 image.
...
Build the Go image from the repository root with `Dockerfile_go`, then set `RAGFLOW_IMAGE` in **docker/.env-go**. See the [quickstart guide](./quickstart.mdx#start-up-the-server) for the current commands and prerequisites.
...
Check that the Go API and admin services started, then review the RAGFlow container and Nginx logs. In the Go configuration, Nginx forwards API requests to port `9384` and admin requests to port `9383`. Check the active `ragflow.conf` against `docker/nginx/ragflow.conf.golang` and verify that those services are listening before changing the proxy configuration.
...
You cannot log in until the services are initialized. Review the RAGFlow container logs (`docker compose --env-file docker/.env-go -f docker/docker-compose-go.yml logs -f ragflow-cpu` from the repository root when using the CPU Compose service; use `ragflow-gpu` for the GPU service). The Go services log these startup messages:

```text
Starting api server: ...
Server starting on port: 9384
Starting admin server: ...
Starting RAGFlow admin HTTP server on port: 9383
...
1. From the repository root, confirm that Kvrocks is running:
   ```bash
   docker compose --env-file docker/.env-go -f docker/docker-compose-go.yml ps kvrocks
   ```
2. Inspect the Kvrocks logs:
   ```bash
   docker compose --env-file docker/.env-go -f docker/docker-compose-go.yml logs kvrocks
   ```
3. Check the Go service logs for connection-refused, authentication, or DNS errors. The standard Go Compose deployment connects to `kvrocks:6379`; for a custom deployment, verify that `KVROCKS_HOST` and `KVROCKS_PORT` point to a reachable service.
...
1. Check the RAGFlow container logs for API, admin, and ingestor startup errors:
   ...
   ```bash
   docker compose --env-file docker/.env-go -f docker/docker-compose-go.yml logs -f ragflow-cpu
   ```
...
To view the container's combined output, use `docker compose --env-file docker/.env-go -f docker/docker-compose-go.yml logs -f ragflow-cpu` (or `ragflow-gpu` for a GPU deployment).
...
1. Check the status of the Elasticsearch Docker container:
   ...
   ```bash
   docker compose --env-file docker/.env-go -f docker/docker-compose-go.yml ps es01
   ```
...
For normal UI and API access, use `http://<IP_OF_YOUR_MACHINE>` when `SVR_WEB_HTTP_PORT` is 80, or include the configured web port. Nginx forwards API traffic to the Go API on internal port `9384` and admin traffic to internal port `9383`.

The CPU Compose service also publishes these ports directly through `GO_HTTP_PORT` and `GO_ADMIN_PORT`. The GPU Compose service does not publish them directly, so access it through the Nginx web port unless you add explicit port mappings. Do not use `SVR_HTTP_PORT` or `ADMIN_SVR_HTTP_PORT` to reach the Go servers; those variables publish compatibility ports on which the Go processes do not listen.
...
4. Set `RAGFLOW_IMAGE` in **docker/.env-go** to a compatible Go image. If you build from source, build `Dockerfile_go`, tag the image, and use that tag as `RAGFLOW_IMAGE`.
5. Pull the configured images when using registry images, then start the deployment:
   ```bash
   docker compose --env-file docker/.env-go -f docker/docker-compose-go.yml pull
...
2. Set the following value in **docker/.env-go**. Its `COMPOSE_PROFILES` setting includes `DOC_ENGINE`, so this selects the Infinity service when the deployment starts:
...
Uploaded files are stored in the configured object storage backend. The current Go backend uses MinIO by default and also supports Amazon S3, Alibaba Cloud OSS, and Google Cloud Storage. Azure Blob Storage and OpenDAL are not currently implemented by the Go storage factory.
...
The Go ingestor currently uses a fixed document indexing batch size of 32. `DOC_BULK_SIZE` does not change it. Embedding request size comes from the model's `batch_size` capability, or defaults to 16 when none is set. For local tuning, `TOKENIZER_EMBEDDING_BATCH_SIZE` overrides that value when set to a positive integer. `EMBEDDING_BATCH_SIZE` does not control this Go path. Larger embedding batches can use more memory; check the model provider's limits before increasing the value.
...
2. In **docker/.env-go** or on the **Model providers** page in the UI, configure RAGFlow as a remote client to MinerU:
...
3. Configure the following in **docker/.env-go** (or your shell if running from source):
...
The Go parser uses a remote Docling Serve endpoint. Set the endpoint in the parser configuration (`docling_server_url`) or provide it through `DOCLING_SERVER_URL` in **docker/.env-go**. If the service requires bearer authentication, also set `docling_api_key` in the parser configuration or provide `DOCLING_API_KEY`:
