...
### Why don't you use other open-source vector databases as document engine?

Currently, only Elasticsearch and [Infinity](https://github.com/infiniflow/infinity) meet the hybrid search requirements of RAGFlow. Most open-source vector databases have limited support for full-text search, and sparse embedding is not an alternative to full-text search. Further, these vector databases do not support features crucial for RAGFlow, such as phrase search and ranking capabilities.

This motivated us to develop an AI-native database [Infinity](https://github.com/infiniflow/infinity) from the ground up.
