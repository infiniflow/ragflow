...
Ollama may time out or fail during its first model load due to timeout or out-of-memory (OOM). It is best to test it in isolation first. If sharing hardware with other services, memory exhaustion is likely. To resolve this, switch to a smaller model or increase RAM.
