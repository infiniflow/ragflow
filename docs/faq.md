...
For a locally deployed RAGFlow: the file size limit each upload is 1GB, with a batch upload limit of 32 files and no cap on the total number of files per account. To update this 1GB file size limit:
...
:::tip NOTE
It is not recommended to manually change the 32-file batch upload limit. However, if you use RAGFlow's HTTP API and Python SDK to upload files, the 32-file batch upload limit is automatically removed.
:::
