...
Selector не опирается только на расширение файла. Изменение registry, global configuration или контракта расширяет набор по потребителям. T3 bootstrap реализует exact path/prefix/suffix selector, использует свежий T0 и базовую карту для удалённых/переименованных путей и сохраняет причину `NOT_APPLICABLE`. Workflow запускается всегда, но пока имеет режим `REPORT_ONLY / NOT_ENABLED`: required branch protection и trusted base-policy execution ещё не подтверждены.
