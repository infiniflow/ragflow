...
Предлагаемый runner `tools/quality/check_architecture.py` появится на T2. Он использует нативные анализаторы из архитектурного документа и собирает их результаты; не заменяет их текстовым поиском всех языков.
