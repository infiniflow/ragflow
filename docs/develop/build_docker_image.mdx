...
Open the `docker/.env` file. Find the `RAGFLOW_IMAGE` setting and change the image reference from `infiniflow/ragflow:v0.27.1` to `infiniflow/ragflow:nightly` to use the pre-built image.
