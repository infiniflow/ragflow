...
- **Linux x86-64:** CPU deployment is the default and recommended path. GPU deployment is available when the host has NVIDIA Container Toolkit configured.
...
The default `DEVICE=cpu` starts `ragflow-cpu`. To use `ragflow-gpu`, set `DEVICE=gpu` in `.env-go` and configure the NVIDIA container runtime on the host.
