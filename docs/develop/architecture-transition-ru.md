...
Дата обновления: 2026-09-10. Статус: T0 реализован для снимка из [отчёта инвентаризации](t0-provenance-report-ru.md); T1 зафиксирован как `BASELINE_COMPLETE_WITH_KNOWN_FAILURES` в [отчёте регресса](t1-regression-report-ru.md): локальные Docker prerequisites закрыты, а реальные MRZ и persisted-DSL сбои локализованы и блокируют рефакторинг только этих границ. T2 принят как `REPORT_ONLY_COMPLETE_WITH_CLASSIFIED_LIMITS`: реализованы [наблюдательный Python-анализ, точные runtime-контракты и первый DEAD-01 scan/plan](t2-python-analysis-ru.md), exact contracts локально затронутых frontend routes/navigation, bootstrap/provider/router composition и HTTP service factories, [широкий TypeScript observer](t2-typescript-analysis-ru.md), [широкий Go observer](t2-go-analysis-ru.md), [классификация их runtime roots и сигналов](t2-runtime-graph-classification-ru.md), а также [Go build profiles](t2-go-build-profiles-ru.md). Это завершение инструментария и классификации T2, а не архитектурный/dead-code/build PASS. Linux/native lane выполнен для предыдущего зафиксированного T0 snapshot; текущий зафиксированный snapshot имеет свежий host-план, но не новый `--execute`. Для T3 локально реализован [report-only base-policy protocol](t3-architecture-policy-ru.md): authoritative evaluator извлекается из comparison base после появления protocol v1, candidate policy не может сузить его selectors/rules/fixtures, protected producers материализуются отдельно и запускаются с isolated import path, а fresh final job изолирован от candidate analysis. Runtime probes пока не имеют OS-level isolation от намеренно враждебного candidate code. Текущий base содержит только предыдущий bootstrap, поэтому первый переход всё ещё маркируется `candidate-bootstrap / NOT_ENABLED`; реальный CI run и Required Workflow ruleset не подтверждены. T3–T7 не приняты.
