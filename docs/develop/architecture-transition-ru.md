...
Дата обновления: 2026-09-10. Статус: T0 реализован для снимка из [отчёта инвентаризации](t0-provenance-report-ru.md); T1 зафиксирован как `BASELINE_COMPLETE_WITH_KNOWN_FAILURES` в [отчёте регресса](t1-regression-report-ru.md): локальные Docker prerequisites закрыты, а реальные MRZ и persisted-DSL сбои локализованы и блокируют рефакторинг только этих границ. T2 принят как `REPORT_ONLY_COMPLETE_WITH_CLASSIFIED_LIMITS`: реализованы [наблюдательный Python-анализ, точные runtime-контракты и первый DEAD-01 scan/plan](t2-python-analysis-ru.md), exact frontend contracts, [широкий TypeScript observer](t2-typescript-analysis-ru.md), [широкий Go observer](t2-go-analysis-ru.md), [классификация runtime roots и сигналов](t2-runtime-graph-classification-ru.md), а также [Go build profiles](t2-go-build-profiles-ru.md). Это завершение инструментария и классификации T2, а не архитектурный/dead-code/build PASS. Для T3 [report-only base-policy protocol](t3-architecture-policy-ru.md) уже вошёл в `origin/main` через merge commit `db7d6cfc22b74853790b9f5fd21a55756cc56e86`. Реальный GitHub-hosted bootstrap run выполнил plan и дошёл до analysis, где выявил дефект выбора Python launcher; browser workflow выявил отдельные receipt/WebKit/cache дефекты. Текущий follow-up кандидат исправляет их и проверен локально, но base-mode GitHub evidence, OS-level runtime isolation, enforced control-owner approval и Required Workflow ruleset ещё не подтверждены. T3–T7 не приняты.
