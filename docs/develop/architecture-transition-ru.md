...
Дата обновления: 2026-09-06. Статус: T0 реализован для снимка из [отчёта инвентаризации](t0-provenance-report-ru.md); T1 имеет выполненные проверки и открытые условия приёмки в [отчёте регресса](t1-regression-report-ru.md). Начат наблюдательный Python-пилот [T2](t2-python-analysis-ru.md); полный T2 и T3–T7 ещё не приняты.
...
Созданы реестры происхождения T0 и утилита их снимка, добавлен ограниченный Python import/complexity отчёт. Полный анализ архитектуры/мёртвого кода, новые CI gates и branch protection ещё не внедрены. Проверки поведения приложения не объявляются пройденными по результату инвентаризации или статического отчёта.
...
**Текущий результат.** [Прогоны, добавленные проверки и незакрытые границы](t1-regression-report-ru.md), [машинные результаты с test IDs](t1-regression-results.json). На кандидате C проверены перенос выбранных данных v1.12.0, отказ DDL, PostgreSQL dump/restore, независимый MinIO restore и реальные ASR/LLM сценарии. Сбои C и отдельные проверки последующих исправлений тестового стенда сохранены. Это не закрывает внешние интеграции, согласованный backup всей поставки и остальные перечисленные границы. Перед изменением конкретной границы надо проверить её ограничения в отчёте; T1 целиком не объявлен завершённым.
...
**Текущий результат.** [Python-пилот и инструкции](t2-python-analysis-ru.md): AST-граф с неявными parent initializers, диагностикой неизвестной динамики, provenance и сигналами сложности. Он не импортирует приложение и не применяет patch. Нет полного allowlist, dead-code verdict, TS/Go анализа или CI gate. Это ранняя подготовка инструментов; сбои и пробелы T1 не скрываются её результатом.

**Действия.** Реализовать схему реестров, классификацию изменений, import analysis и отчёт. Настроить Python dynamic registrations, frontend entrypoints и Go build profiles. Анализатор работает в режиме отчёта; existing tests продолжают блокировать регресс как раньше.
