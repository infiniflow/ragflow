...
- A recommended starting configuration of 4 CPU cores (x86-64), 16 GB RAM,
  and 50 GB free disk space. Actual requirements depend on the document engine,
  data volume, parsing workload, and concurrency. Local models and OceanBase
...
3. Set `RAGFLOW_IMAGE=ragflow:go-local` in `docker/.env`. The same file controls `DOC_ENGINE`, `DEVICE`, service ports, and dependency credentials. Change the default passwords before deploying to a network-accessible host.
...
4. Start the Go service and its dependencies:
   ...
   The default `DEVICE=cpu` selects `ragflow-cpu`. For the GPU Compose service, set `DEVICE=gpu` in `.env` and configure the NVIDIA container runtime on the host. In the RAGFlow open-source 1.0 release, DeepDoc layout analysis, OCR, and table recognition use CPU inference; selecting `DEVICE=gpu` does not change these DeepDoc operations.
...
5. Check that the service is running:
   ...
   docker compose --env-file .env -f docker-compose.yml logs -f ragflow-cpu
   ```
   ...
   This check works for both CPU and GPU deployments. A successful response has HTTP status 200. If you changed `SVR_WEB_HTTP_PORT` in `.env`, include that port in the URL. Wait for this check to pass before opening the web interface.

6. Open `http://IP_OF_YOUR_MACHINE` in a browser and log in. If you changed `SVR_WEB_HTTP_PORT` in `.env`, include that port in the URL.
