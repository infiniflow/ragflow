...
4. Start the Go service and its dependencies:
   ...
   The default `DEVICE=cpu` selects `ragflow-cpu`. For the GPU Compose service, set `DEVICE=gpu` in `.env-go` and configure the NVIDIA container runtime on the host. In the RAGFlow open-source 1.0 release, the Go DeepDoc backend is CPU-only and does not support GPU inference; selecting `DEVICE=gpu` does not accelerate DeepDoc layout analysis, OCR, or table recognition.
