...
4. Start the Go service and its dependencies:
   ...
   The default `DEVICE=cpu` selects `ragflow-cpu`. For the GPU Compose service, set `DEVICE=gpu` in `.env-go` and configure the NVIDIA container runtime on the host.
