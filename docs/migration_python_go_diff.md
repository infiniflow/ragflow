...
| # | Item | Impact | Sev | Diff | Effort | Status |
|---|------|--------|-----|------|--------|--------|
...
| # | Item | Cat | Impact | Sev | Diff | Effort | Status |
|---|------|-----|--------|-----|------|--------|--------|
...
| 2.3 | missing pos handling | D | `nlp/__init__.py:1174-1192` | `token.go:771` | **Fixed** (2026-07-24, line re-verified 2026-07-27) — `mergeByTokenSizeFromJSON` (`token.go:771`) now extends `Positions` (and `_pdf_positions`) across merges via `extendRawJSONArray`, mirroring `token_chunker.py:240`. Same root cause as 2.5; coord→position_int conversion is owned by `internal/ingestion/task` `processChunkPositions`. | P1 |
...
| 2.8 | Group/Hierarchy missing `remove_tag` | D | `common.py:255` | `group.go:224-231` | **Fixed** (2026-07-24) — `buildChunksFromRecordGroups` (shared by Group+Hierarchy) now calls `removeTag` on the joined text; the `merge_pdf_positions` part is diff 1.6. | P1 |
...
| 4 | `_ERROR` output mode | — | Python fail-soft vs Go returns error | P3 | Mid | M | **Keep Go** — Go returns error (fail-loud); Python's `_ERROR` output channel has no equivalent in the Go pipeline. By design, not a gap |
...
1. **Wave 3 (LLM robustness)** — resolved: Extractor omission 6 (retry) and diff 8 (json_repair) are already **Fixed**.
2. **Wave 4 (data paths)** — diff 2 (tag_kb_ids) only; diff 13 ({key} substitution) already **Fixed**.
3. **Wave 5 (P2/P3 batch)** — diffs 5/6/10/17 + omission 8 (omission 7 already **Fixed**; diff 14 is **Keep Go**, not a gap).
...
| Parser | 0 | 0 | 0 | 0 | — |
| Chunker | 0 | 0 | 0 | 0 | — |
| Extractor | 0 | 1 | 4 | 1 | ~2 days |
| Tokenizer | 0 | 0 | 0 | 0 | — |
| **Total** | **0** | **1** | **4** | **1** | **~2 days** |

Counts reflect open items only ("Fixed" / "Not a gap" / "Keep Go" / "Deferred" excluded). Extractor open items: diff 2 = P1×1; diffs 5/6/10/17 = P2×4; omission 8 = P3×1. (Previously listed omission 6, diff 8, diff 13 as open P1 — all three are now **Fixed** and removed.) Deferred to colleague: diff 1 (P0 tag matching, ES integration). Deferred to compilation layer: omission 2 (metadata), omission 3 (TOC). Keep Go: omission 1 (cache — checkpoint covers this), omission 5 (message_fit_in — truncation belongs at Chunker layer).
